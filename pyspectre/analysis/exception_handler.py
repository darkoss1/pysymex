...
class ExceptionHandlerState:
    ...
    active_handlers: list[ExceptionHandlerInfo] = field(default_factory=list)

    all_handlers: list[ExceptionHandlerInfo] = field(default_factory=list)
...
class ExceptionHandlerAnalyzer:
    ...
    def _parse_exception_table(self, code: object) -> list[ExceptionHandlerInfo]:
        ...
        try:
            ...
            instructions = list(dis.get_instructions(code))
            offsets = {instr.offset: i for i, instr in enumerate(instructions)}
                ...
                if child.finalbody:
                    handlers.append(
                        ExceptionHandlerInfo(
                            ...
                            end_pc=(
                                child.finalbody[-1].end_lineno if child.finalbody else child.lineno
                            ),
                            nesting_depth=depth,
                        )
                    )
                ...
                if child.orelse:
                    handlers.append(
                        ExceptionHandlerInfo(
                            ...
                            end_pc=child.orelse[-1].end_lineno if child.orelse else child.lineno,
                            nesting_depth=depth,
                        )
                    )
