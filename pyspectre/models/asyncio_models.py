...
class CoroutineModel(Generic[T]):
    ...
    def throw(
        self, typ: type[BaseException], val: BaseException | None = None, tb: Any = None
    ) -> T:
...
class TaskModel(Generic[T]):
    ...
    _callbacks: list[Callable[[TaskModel], None]] = field(default_factory=list)
    ...
    def add_done_callback(self, callback: Callable[[TaskModel], None]) -> None:
    ...
    def remove_done_callback(self, callback: Callable[[TaskModel], None]) -> int:
    ...
    def set_name(self, value: str) -> TaskModel:
...
class EventModel:
    ...
    _set: bool = False
    _waiters: list[Any] = field(default_factory=list)
...
class FutureModel(Generic[T]):
    ...
    def __init__(self) -> None:
        ...
        self._callbacks: list[Callable[[FutureModel], None]] = []
    ...
    def add_done_callback(self, callback: Callable[[FutureModel], None]) -> None:
    ...
    def remove_done_callback(self, callback: Callable[[FutureModel], None]) -> int:
        """Remove a callback from the done callbacks list."""
        count = self._callbacks.count(callback)
        while callback in self._callbacks:
            self._callbacks.remove(callback)
...
ASYNCIO_MODELS = {
    ...
    "Future": FutureModel,
    "sleep": lambda delay: type("sleep_coro", (), {"__await__": lambda self: (yield None)})(),
