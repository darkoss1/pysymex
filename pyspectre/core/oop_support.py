"""
Enhanced OOP Support for PySpectre v1.2.
...
class InitParameter:
    ...
    def to_symbolic(self, pc: int) -> Any:
        ...
        type_map = {
            ...
            "str": lambda: SymbolicValue.symbolic(f"init_{self.name}_{pc}")[0],
            "float": lambda: SymbolicValue.symbolic(f"init_{self.name}_{pc}")[0],
            "bool": lambda: SymbolicValue.symbolic(f"init_{self.name}_{pc}")[0],
            "list": lambda: SymbolicValue.symbolic(f"init_{self.name}_{pc}")[0],
            "dict": lambda: SymbolicValue.symbolic(f"init_{self.name}_{pc}")[0],
        }
...
class EnhancedClass:
    ...
    static_methods: dict[str, EnhancedMethod] = field(default_factory=dict)
    properties: dict[str, SymbolicProperty] = field(default_factory=dict)
    init_params: list[InitParameter] = field(default_factory=list)
    required_init_args: int = 0
    class_vars: dict[str, Any] = field(default_factory=dict)
...
class EnhancedObject:
    ...
    def call_method(
        ...
        if not found:
            return None, False
        if isinstance(value, EnhancedMethod):
            call_args, call_kwargs = value.get_call_args(args, kwargs)
