...
class PySpectreConfig:
    ...
    def to_toml(self) -> str:
        ...
        lines.append("[tool.pyspectre.detectors]")
        ...
        lines.append("")
        lines.append("[tool.pyspectre.limits]")
        for key, value in self.limits.to_dict().items():
            lines.append(f"{key} = {value}")
        lines.append("")
        lines.append("[tool.pyspectre.output]")
        for key, value in self.output.to_dict().items():
            if isinstance(value, bool):
                ...
                lines.append(f"{key} = {value}")
        lines.append("")
        lines.append("[tool.pyspectre.analysis]")
...
def find_config_file(start_dir: Path | None = None) -> Path | None:
    ...
    home = Path.home()
    for config_name in [".pyspectre.toml", "pyspectre.toml"]:
...
def load_config(
    ...
    if config_path.name == "pyproject.toml":
        shadow_data = data.get("tool", {}).get("pyspectre", {})
    else:
...
def init_config(directory: Path | None = None) -> Path:
    ...
    if directory is None:
        directory = Path.cwd()
    config_path = directory / "pyspectre.toml"
    if config_path.exists():
