...
from pysymex.core.cache.control import is_process_cache_disabled
from pysymex.sandbox.bridge.cache import MODULE_CACHE, module_cache_key, remember_module_blob
from pysymex.config.sandbox_bridge import to_int
from pysymex.sandbox.bridge.module.worker import build_module_worker
from pysymex.sandbox.bridge.types import ExtractionLimits, ModuleBlob
from pysymex.sandbox.bridge.workers import run_json_worker as _run_json_worker
...
def extract_module(
    ...
    *,
    ...
    if not isinstance(payload_obj, dict):
        ...
        raise SandboxProtocolError("Sandbox worker produced no module payload")
    ...
    if len(payload) > max_result_bytes:
        ...
        raise SandboxResourceError("Sandbox module payload exceeds configured result size limit")
    blob = ModuleBlob(
        payload=payload,
        filename=filename,
