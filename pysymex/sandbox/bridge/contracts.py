...
from pysymex.sandbox.bridge.schema import (
    ...
    require_json_object,
    require_json_str,
    UnsupportedSandboxCallableContract,
)
...
def decode_optional_function_contract(
    ...
    *,
    ...
    from pysymex.contracts.types import ContractKind, FunctionContract

    data = require_json_object(value, "target.contract")
    reject_unexpected_fields(data, FUNCTION_CONTRACT_KEYS, "target.contract")

    declared_name = require_json_str(
        data["function_name"],
...
def _decode_contract_clauses(
    ...
    *,
    ...
    """
    from pysymex.contracts.types import Contract, ContractKind, ContractPredicate, Severity

    clauses: list[Contract] = []
    raw_clauses = require_json_list(value, context, limits.max_consts)
    for index, item in enumerate(raw_clauses):
        clause_context = f"{context}[{index}]"
        data = require_json_object(item, clause_context)
        reject_unexpected_fields(data, FUNCTION_CONTRACT_CLAUSE_KEYS, clause_context)
        kind_name = require_json_str(
            data["kind"],
        ...
        if kind is None:
            raise ValueError(f"{clause_context}.kind: unknown contract kind {kind_name!r}")
        if kind is not expected_kind:
            raise ValueError(f"{clause_context}.kind: unexpected contract kind {kind_name!r}")
        predicate_kind = require_json_str(
            data["predicate_kind"],
            f"{clause_context}.predicate_kind",
            max_length=limits.max_name_length,
        )
        condition = require_json_str(
            data["condition"],
            f"{clause_context}.condition",
            max_length=limits.max_string_length,
        )
        if predicate_kind == "string":
            predicate = require_json_str(
                data["predicate"],
            ...
            predicate = cast(
                "ContractPredicate",
                UnsupportedSandboxCallableContract(condition=condition),
            )
        else:
            raise ValueError(
                f"{clause_context}.predicate_kind: unsupported predicate kind {predicate_kind!r}"
            )
        ...
        severity_name = require_json_str(
            data["severity"],
            f"{clause_context}.severity",
            max_length=limits.max_name_length,
        )
        severity = Severity.__members__.get(severity_name)
        if severity is None:
            raise ValueError(f"{clause_context}.severity: unknown severity {severity_name!r}")
        line_value = data["line_number"]
        if line_value is None:
            line_number = None
        else:
            line_number = require_json_int(
                line_value,
...
def make_cell(value: object) -> CellType:
    ...
    if closure is None:
        raise ValueError("Failed to create closure cell")
    return closure[0]
