...
from pysymex.sandbox.bridge.payload import (
    PayloadParseStatus,
    extract_payload,
...
def run_json_worker(
    ...
    *,
    ...
    Side Effects:
        ...
        `execute_code`.
    """
    from pysymex.sandbox import SandboxResult, SecureSandbox
        ...
        if parsed is None:
            if sandbox_result.succeeded:
                if parse_status == PayloadParseStatus.MULTIPLE:
                    raise SandboxProtocolError("Sandbox worker produced multiple result payloads")
                if parse_status == PayloadParseStatus.MALFORMED:
                    raise SandboxProtocolError("Sandbox worker produced malformed result payload")
