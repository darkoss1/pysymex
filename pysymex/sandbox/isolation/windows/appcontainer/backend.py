...
from ....types import SandboxResult, SecurityCapabilities
from ...base import IsolationBackend
from ...constants import HARNESS_FILENAME
from ...harness import generate_harness_script
from .acl import WindowsAclEnvironmentMixin, make_profile_name
from .process import WindowsNativeProcessMixin
from .runtime import WindowsRuntimeCacheMixin
from .self_check import WindowsSelfCheckMixin
from ..native.api import WindowsNativeApi, has_windows_native_appcontainer_support
...
class WindowsAppContainerBackend(
    ...
    def setup(self) -> None:
        ...
        if not self.is_available:
            ...
            raise SandboxSetupError("Windows AppContainer APIs are not available")
        ...
        try:
            ...
            if self._jail_path is None:
                raise SandboxSetupError("Windows AppContainer jail was not created")
            ...
            logger.verbose(
                "Windows AppContainer sandbox verified for profile %s", self._profile_name
            )
            ...
            raise SandboxSetupError(
                f"Failed to set up Windows AppContainer sandbox: {exc}"
            ) from exc
    ...
    def reset_workspace(self) -> None:
        ...
        for child in self._jail_path.iterdir():
            ...
            if child_resolved != jail_path and jail_path not in child_resolved.parents:
                raise SandboxSetupError(
                    f"Refusing to reset path outside AppContainer jail: {child}"
                )
            if child.is_dir():
                shutil.rmtree(child)
            else:
    ...
    def _create_configured_job_object(self) -> int:
        ...
        if self._win32 is None:
            raise SandboxSetupError("Windows AppContainer Win32 API wrapper is unavailable")
...
__all__ = ["WindowsAppContainerBackend", "has_windows_appcontainer_support"]
