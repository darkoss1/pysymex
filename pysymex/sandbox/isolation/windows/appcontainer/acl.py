...
class WindowsAclEnvironmentMixin:
    ...
    def _grant_jail_access(self) -> None:
        ...
        if self._jail_path is None or self._appcontainer_sid_string is None:
            raise SandboxSetupError("AppContainer jail or SID is unavailable")
        self._run_icacls(
            [
                str(self._jail_path),
                "/grant",
    ...
    def _run_icacls(self, args: list[str], *, context: str) -> None:
        ...
        if not icacls.is_file():
            ...
            raise SandboxSetupError(f"icacls.exe is required to {context}")
        ...
        if completed.returncode != 0:
            ...
            raise SandboxSetupError(
                f"Failed to {context}: {completed.stderr.strip() or completed.stdout.strip()}"
            )
