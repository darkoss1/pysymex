...
class WindowsNativeTokenMixin:
        ...
        *,
        ...
        try:
            ...
            if int(is_appcontainer.value) != 1:
                raise SandboxSetupError("Created process token is not AppContainer-backed")
    ...
    def _verify_lpac(self, token: int) -> None:
        ...
        if int(is_lpac.value) != 1:
            raise SandboxSetupError("Created process token is not LPAC-backed")
    ...
    def _verify_appcontainer_sid(
        ...
        *,
        ...
        if not info.TokenAppContainer:
            raise SandboxSetupError("Created process token has no AppContainer SID")
        if expected_appcontainer_sid is not None and not self.advapi32.EqualSid(
            ...
            raise SandboxSetupError("Created process AppContainer SID does not match profile")
    ...
    def _verify_expected_capability_count(self, token: int) -> None:
        ...
        if int(group_count) != len(LPAC_CAPABILITY_NAMES):
            raise SandboxSetupError(
                "Created process token has unexpected AppContainer capability count"
            )
    ...
    def _verify_low_integrity(self, token: int) -> None:
        ...
        if not count_ptr:
            raise SandboxSetupError("Could not inspect process integrity SID")
        count = int(count_ptr.contents.value)
        if count <= 0:
            raise SandboxSetupError("Process integrity SID has no subauthorities")
        rid_ptr = self.advapi32.GetSidSubAuthority(sid, count - 1)
        if not rid_ptr:
            raise SandboxSetupError("Could not inspect process integrity RID")
        if int(rid_ptr.contents.value) > SECURITY_MANDATORY_LOW_RID:
            raise SandboxSetupError("AppContainer process is not low/untrusted integrity")
