...
from pysymex.benchmarks.suite.types import BenchmarkEvent, BenchmarkResult, BenchmarkStatus
...
class BenchmarkReporter:
    ...
    def to_console(results: list[BenchmarkResult]) -> None:
        ...
        logger.verbose("Rendering %d benchmark result(s) to console", len(results))
    ...
    def format_inventory(results: list[str]) -> str:
        ...
        if not results:
            return "No benchmark cases matched the requested filters."
        lines = ["Available benchmark cases:"]
    ...
    def progress_line(event: BenchmarkEvent) -> str:
        ...
        prefix = f"[BENCH] [{event.completed}/{event.total}]"
    ...
    @staticmethod
    def print_progress(event: BenchmarkEvent, *, stream: TextIO | None = None) -> None:
        """Print live benchmark progress to stdout or stderr."""
        target = stream if stream is not None else sys.stdout
        print(BenchmarkReporter.progress_line(event), file=target)
