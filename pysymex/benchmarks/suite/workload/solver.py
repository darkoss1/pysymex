...
from pysymex.benchmarks.suite.workload.inputs import (
...
def bench_simple_arithmetic() -> dict[str, int]:
    ...
    return {
        ...
        "paths": 1,
        "solver_calls": solver_queries_from_stats(stats),
        **solver_outcome_counts_from_stats(stats),
    }


def bench_solver_sat_cache_hits() -> dict[str, int]:
...
def bench_solver_unsat_subset_cache_hits() -> dict[str, int]:
    ...
    for index in range(supersets):
        result = solver.check_sat_result([*contradiction, y == index % 3])
...
def bench_solver_exact_literal_cache_hits() -> dict[str, int]:
    """Benchmark: exact Boolean literal classification across Z3 wrapper objects."""
    import z3

    from pysymex.core.solver.constraints.literals import (
