...
def evaluate_capsule_dominance_action(
    action: EvidenceAction,
    subject: ObligationCapsule,
    candidates: Iterable[ObligationCapsule],
) -> EvidenceOutcome:
    """Reject capsule-only dominance when exact checkpoint evidence is unavailable."""
    if (
    ...
    if action.capsule_id != subject.capsule_id:
        return EvidenceOutcome(
            action=action,
            kind=EvidenceOutcomeKind.INCONCLUSIVE,
    ...
    return EvidenceOutcome(
        action=action,
        kind=EvidenceOutcomeKind.INCONCLUSIVE,
        explanation="checkpoint evidence is required for dominance removal",
    )


def evaluate_checkpoint_dominance_action(
    action: EvidenceAction,
...
def evaluate_checkpoint_unsat_core_action(
    action: EvidenceAction,
