...
def handle_return_const(
    ...
    if frame is not None:
        ...
        if not getattr(frame, "is_init_call", False):
            state = state.push(return_value)
        else:
            init_instance = getattr(frame, "init_instance", None)
            if init_instance is not None:
                state = state.push(init_instance)
            else:
...
def handle_pop_jump_if_true(
    ...
    # POP_JUMP_IF_TRUE: true condition jumps to target, false falls through
    # POP_JUMP_IF_FALSE: true condition falls through, false jumps to target
    if instr.opname == "POP_JUMP_IF_TRUE":
        true_pc = target_index
        false_pc = state.pc + 1
    else:  # POP_JUMP_IF_FALSE
        true_pc = state.pc + 1
        false_pc = target_index
