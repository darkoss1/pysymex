...
def normalize_construction_protocol_return(
    frame: CallFrame,
    ...
    may_be_object = modeled_object is not None or not z3.is_false(z3.simplify(return_value.is_obj))
    ...
    if isinstance(modeled_object, SymbolicInstance) and z3.is_true(
        z3.simplify(return_value.is_obj)
    ):
...
def continue_modeled_instance_initialization(
    ...
    if (
        ...
        or not z3.is_true(z3.simplify(return_value.is_obj))
...
def _constructor_call(
    ...
    if isinstance(retained, tuple):
        payload = cast("tuple[object, ...]", retained)
    else:
        payload = ()
    if len(payload) == 3:
...
__all__ = ["continue_modeled_instance_initialization", "normalize_construction_protocol_return"]
