...
class DeclaredDescriptorBinding:
...
def register_declared_descriptor_bindings(
    modeled_cls: SymbolicClass, owner_value: SymbolicValue
) -> None:
    ...
    retained: dict[str, DeclaredDescriptorBinding] = {}
    for attr_name, assignment in cast("dict[object, object]", assignments).items():
        ...
        retained[attr_name] = DeclaredDescriptorBinding(
    ...
    if retained:
        setattr(modeled_cls, "_pysymex_declared_descriptors", retained)
...
def find_declared_descriptor_binding(
    receiver: SymbolicValue, attr_name: str
) -> DeclaredDescriptorBinding | None:
...
def _find_binding_in_mro(
    modeled_cls: SymbolicClass, attr_name: str
) -> DeclaredDescriptorBinding | None:
    ...
    for candidate_cls in modeled_cls.mro:
        ...
        if isinstance(bindings, dict) and attr_name in bindings:
            return cast("DeclaredDescriptorBinding", bindings[attr_name])
    return None


def _synthesized_set_name_attributes(
    method: object, attr_name: str, owner_value: SymbolicValue
...
def _synthesized_init_attributes(
    method: object | None, constructor_args: tuple[object, ...] | None
) -> dict[str, object] | None:
...
def _synthesized_direct_assignments(
    code: types.CodeType, descriptor_name: str, supported_values: dict[str, object]
) -> dict[str, object] | None:
    """Recognize direct ``self.field = parameter`` assignments only."""
    meaningful = [
        instruction
        for instruction in cached_get_instructions(code)
...
def _body_is_bounded(
    method: object, method_name: str, synthesized_attributes: frozenset[str]
) -> bool:
    """Return whether a descriptor method body only touches safe synthesized attributes."""
    code = _method_code(method)
    if code is None:
        return False
    parameters = code.co_varnames[: code.co_argcount]
    if not parameters:
        return False
    descriptor_name = parameters[0]
    owner_name = parameters[2] if method_name == "__get__" and len(parameters) > 2 else None
    ...
    for index, instruction in enumerate(instructions):
        if instruction.opname == "LOAD_FAST_LOAD_FAST":
            ...
            if not _descriptor_attribute_read_is_safe(
                instructions[index + 1], synthesized_attributes
            ):
                return False
            continue
        if instruction.opname != "LOAD_FAST":
            continue
        if owner_name is not None and instruction.argval == owner_name:
            return False
...
def _descriptor_attribute_read_is_safe(
    instruction: dis.Instruction, synthesized_attributes: frozenset[str]
) -> bool:
