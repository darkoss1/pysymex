...
def class_body_function_payload(
    ...
    payload = _payload_from_make_function(instructions, code, closure_by_name)
    if payload is None:
        return _payload_with_closure(code, closure_by_name)

    if payload.closure:
        return payload
    if not code.co_freevars or not closure_by_name:
        return payload if _has_call_semantic_metadata(payload) else code

    closure = _closure_from_names(code, closure_by_name)
    if closure is None:
        return payload if _has_call_semantic_metadata(payload) else code
    return replace(payload, closure=closure)


def _payload_from_make_function(
    instructions: Sequence[dis.Instruction],
    code: types.CodeType,
    closure_by_name: Mapping[str, object] | None,
    ...
    boundary = _previous_class_body_store_index(instructions, make_index)
    stack: list[object] = []
    for index, instr in enumerate(instructions[boundary + 1 :], start=boundary + 1):
        if instr.opname == "LOAD_CONST":
            ...
            _build_tuple(stack, int(instr.arg or 0))
        ...
        if stack and isinstance(stack[-1], SymbolicFunctionPayload):
            ...
            if next_op not in {"SET_FUNCTION_ATTRIBUTE"}:
                break
...
def _build_tuple(stack: list[object], count: int) -> None:
...
def _closure_operand(
    ...
    if not isinstance(raw_name, str) or closure_by_name is None:
        return _UNKNOWN
    return closure_by_name.get(raw_name, _UNKNOWN)
...
def _has_call_semantic_metadata(payload: SymbolicFunctionPayload) -> bool:
    """Return whether payload metadata can affect call binding or closure resolution."""
    return bool(payload.defaults or payload.kwdefaults is not None or payload.closure)


def _closure_from_names(
    code: types.CodeType,
    closure_by_name: Mapping[str, object] | None,
) -> tuple[object, ...] | None:
...
__all__ = ["class_body_function_payload"]
