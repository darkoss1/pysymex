...
def unsupported_attribute_protocol_result(state: VMState, *, reason: str) -> OpcodeResult:
    ...
    return OpcodeResult(
        ...
        fallback_events=[unsupported_attribute_protocol_event(state=state, reason=reason)],
        terminal=True,
    )


def unsupported_descriptor_protocol_result(state: VMState, *, reason: str) -> OpcodeResult:
    ...
    return OpcodeResult(
        ...
        fallback_events=[unsupported_descriptor_protocol_event(state=state, reason=reason)],
        terminal=True,
    )


def unsupported_attribute_protocol_event(*, state: VMState, reason: str) -> FallbackEvent:
    """Build a fallback event for unsupported dynamic attribute protocol semantics."""
    return _unsupported_attribute_family_event(
        state=state,
...
def unsupported_descriptor_protocol_event(*, state: VMState, reason: str) -> FallbackEvent:
...
def _unsupported_attribute_family_event(
    *,
    state: VMState,
    label: str,
    reason: str,
