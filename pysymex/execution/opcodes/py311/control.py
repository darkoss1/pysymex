...
def handle_jump_or_pop(
    ...
    branches: list[VMState] = []
    if is_satisfiable(list(jump_state.path_constraints)):
        branches.append(jump_state)
    if is_satisfiable(list(pop_state.path_constraints)):
...
@opcode_handler("JUMP_FORWARD", "JUMP_BACKWARD", "JUMP_BACKWARD_NO_INTERRUPT")
def handle_jump_variations(
    instr: dis.Instruction, state: VMState, ctx: OpcodeDispatcher
) -> OpcodeResult:
    """Handle all unconditional jumps (3.11/3.12+)."""
    target_offset = instr.argval
    assert isinstance(target_offset, int)
    new_pc = ctx.offset_to_index(target_offset)
    if new_pc is None:
        state = state.advance_pc()
    else:
        state = state.set_pc(new_pc)
    return OpcodeResult.continue_with(state)
