...
def _format_value_symbolic(val: object, state: VMState) -> SymbolicString:
    """Helper to format a value symbolically with optimized type checking."""
    if isinstance(val, SymbolicString):
        return val
    if isinstance(val, SymbolicValue):
        affinity = val.affinity_type

        if affinity == "int":
            z3_expr = val.z3_int
        elif affinity == "bool":
            z3_expr = z3.If(val.z3_bool, z3.IntVal(1), z3.IntVal(0))
        elif affinity == "none":
            return SymbolicString.from_const("None")
        else:
            z3_expr = z3.If(
                val.is_int,
                val.z3_int,
        ...
        new_z3_str_expr = z3.If(
            z3_expr < 0, z3.Concat("-", z3.IntToStr(-z3_expr)), z3.IntToStr(z3_expr)
        )
        new_z3_str_expr = z3.If(val.is_none, z3.StringVal("None"), new_z3_str_expr)
        new_z3_str = cast("z3.SeqRef", new_z3_str_expr)

        return SymbolicString(
            _name=f"str({val.name})",
            _z3_str=new_z3_str,
            _z3_len=z3.Length(new_z3_str),
        )
...
def handle_format_value(
    ...
    argval = instr.argval
    flags = 0
    if _is_object_tuple(argval) and len(argval) > 1:
        tuple_flag = argval[1]
        if isinstance(tuple_flag, (int, str)):
            flags = int(tuple_flag)
    elif isinstance(argval, (int, str)) and argval:
        flags = int(argval)
    if flags & 0x04:
        if state.stack:
            state.pop()
...
@opcode_handler("LIST_TO_TUPLE")
def handle_list_to_tuple(
    instr: dis.Instruction, state: VMState, ctx: OpcodeDispatcher
) -> OpcodeResult:
    """Handle LIST_TO_TUPLE."""
    val = state.pop()
    state = state.push(val)
    state = state.advance_pc()
    return OpcodeResult.continue_with(state)
