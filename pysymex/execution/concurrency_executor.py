...
class ConcurrentSymbolicExecutor(SymbolicExecutor):
    ...
    and asyncio, feeding them into ConcurrencyAnalyzer for automated
    ...
    def _execute_step(self, state: VMState) -> None:
        ...
        from pysymex.execution.executor import BRANCH_OPCODES
    ...
    def _intercept_concurrency(self, instr: Any, state: VMState) -> None:
        ...
        try:
            ...
            if self.config.verbose:
                print(f"Concurrency interception error: {e}")
        ...
        if "thread" in name and ("create" in name or "thread(" in name):
            ...
            for _var, tid in self._known_threads.items():
                try:
                    ...
                    if self.config.verbose:
                        print(f"Concurrency thread tracking error (start): {e}")
            ...
            for _var, tid in self._known_threads.items():
                try:
                    ...
                    if getattr(self, "config", None) and getattr(self.config, "verbose", False):
                        print(f"Concurrency thread tracking error (join): {e}")
    ...
    def _finalize_concurrency_analysis(self) -> list[Issue]:
        ...
        try:
            ...
            if getattr(self, "config", None) and getattr(self.config, "verbose", False):
                print(f"Concurrency finalization error: {e}")

        try:
            from pysymex.analysis.interleaving import DPORExplorer

            analyzer_any: Any = self._concurrency_analyzer

            hb_graph: Any = analyzer_any.hb_graph

            thread_ops: Any = analyzer_any.get_thread_operations()
            ...
            if hb_graph and thread_ops and len(thread_ops) > 1:
                ...
                for op1_id, op2_id in race_candidates:
                    op1: Any = hb_graph.get_operation(op1_id)

                    op2: Any = hb_graph.get_operation(op2_id)
                ...
                if self.config.verbose and schedules:
                    print(
                        f"  DPOR explored {len(schedules)} interleavings, "
                        f"found {len(race_candidates)} race candidate(s)"
                    )

        except Exception as e:
            if getattr(self, "config", None) and getattr(self.config, "verbose", False):
                print(f"DPOR exploration error: {e}")
