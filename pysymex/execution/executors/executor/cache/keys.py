...
from pysymex.analysis.runtime.cache import hash_function
...
__all__ = ["execution_result_cache_key"]
...
def _stable_dependency_token(func: Callable[..., object], code: CodeType) -> str:
    ...
    closure = cast("tuple[object, ...] | None", getattr(func, "__closure__", None))
    ...
    for name, value in referenced_same_module_instance_globals(func, code).items():
        referenced_globals[name] = _normalise_value(value)
...
def _normalise_cell(cell: object) -> object:
    """Return a stable representation for a closure cell."""
    try:
        return _normalise_value(getattr(cell, "cell_contents"))
    except ValueError:
