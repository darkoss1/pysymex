...
class VerifiedExecutor:
    ...
    def execute_function(
        ...
        Args:
            ...
            symbolic_args: Mapping of parameter names to type hints
                (e.g. ``{"x": "int"}``).  Defaults to ``"int"``.

        Returns:
            A :class:`VerifiedExecutionResult` containing detected issues,
            contract violations, arithmetic safety proofs, termination
            analysis, and inferred properties.
        """
        import time
        ...
        code = func.__code__
        self._instructions = list(_cached_get_instructions(code))
    ...
    def _detect_loops(self) -> None:
        ...
        for i, instr in enumerate(self._instructions):
            if instr.opname in ("JUMP_BACKWARD", "JUMP_BACKWARD_NO_INTERRUPT"):
                target_offset: object = instr.argval
    ...
    def _extract_contracts(self, func: Callable[..., object]) -> FunctionContract:
    ...
    def _create_symbolic_for_type(self, name: str, type_hint: str) -> object:
    ...
    def _verify_postconditions(self) -> None:
        ...
        for contract in self._function_contract.postconditions:
            ...
            contract_counterexample: dict[str, object] | None = None
    ...
    def _execute_step(self, state: VMState) -> None:
        ...
        if result.issues:
            for issue in result.issues:
                line_no = self._pc_to_line.get(state.pc)
    ...
    def _analyze_single_loop(self, loop_pc: int) -> TerminationProof:
        ...
        if loop_var and loop_var in symbols:
            ...
            if loop_condition == "greater_than_zero":
                ...
                return analyzer.check_termination(
                    z3_condition,
                    loop_body_effect,
                    symbols,
                )
            ...
            if instr.opname == "COMPARE_OP":
                if i >= 2:
                    ...
                    if load_instr.opname == "LOAD_FAST_LOAD_FAST":
                        names: object = load_instr.argval
                        if isinstance(names, tuple) and len(cast("tuple[object, ...]", names)) >= 2:
                            loop_var = str(cast("tuple[object, ...]", names)[0])
                            bound_var = str(cast("tuple[object, ...]", names)[1])
                    else:
...
def verify(
    ...
    **config_kwargs: object,
    ...
    Convenience wrapper that creates a :class:`VerifiedExecutor`,
    runs it on *func*, and returns the result.

    Args:
        func: Function to verify.
        symbolic_args: Mapping of parameter names to types.
        **config_kwargs: Forwarded to :class:`VerifiedExecutionConfig`.

    Returns:
        Verification result with contract and arithmetic status.
...
def check_contracts(
    ...
    """Check function contracts (pre/postconditions).
...
def check_arithmetic(
    ...
    """Check a function for arithmetic safety issues.
    ...
    Returns:
        List of arithmetic issues (division-by-zero, overflow).
    """
    result = verify(
        func,
        symbolic_args,
...
def prove_termination(
    ...
    """Attempt to prove that a function terminates.

    Analyses loops by synthesising ranking functions that
    ...
    Returns:
        A :class:`TerminationProof` with the analysis status.
    """
    result = verify(
        func,
        symbolic_args,
