...
class TerminalEmitter:
    ...
    def _flush(self) -> None:
        ...
        Suppresses OSError exceptions if the stream is closed or detached,
        recording the trace diagnostic event if possible.
        """
        try:
            self._stream.flush()
        except OSError:
            try:
                ...
                get_logger(__name__).trace("Terminal stream flush failed", exc_info=True)
            except Exception:
                pass
