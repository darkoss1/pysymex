...
class ExceptionInfo:
    ...
    def format(self) -> str:
        ...
        return "".join(traceback.format_exception(self.exc_type, self.exc_value, self.traceback))
...
class LogEntry:
    ...
    def format(self, color: bool = True, show_time: bool = True) -> str:
        ...
        from pysymex._internal.logging.formatting import format_log_entry

        return format_log_entry(self, color=color, show_time=show_time)


class LogEntryPolicy:
    """Domain owner for structured log entry construction helpers."""

    @staticmethod
    def exception(exc_info: ExceptionInput) -> ExceptionInfo | None:
