...
Bug class:
    ``RAISE_VARARGS`` on a satisfiable path, indicating
...
Known false-positive conditions:
    ...
    covered by a dedicated assertion detector. Arithmetic bytecode keeps
    ownership of divide-by-zero reports; a bare ``raise`` of a retained
    ``ZeroDivisionError`` remains exception propagation, not a new arithmetic
    operation.
"""
...
from pysymex._internal.core.exceptions.policy import issue_kind_for_explicit_raise
...
class UserExceptionDetector(Detector):
    """Detect explicitly raised exceptions via ``RAISE_VARARGS``."""
    ...
    def check(self, state: VMState, instruction: dis.Instruction, solver: IsSatFn) -> Issue | None:
        ...
        if _is_generator_close_completion_exception(state, exc_name):
            return None
...
def _exception_name(value: object) -> str | None:
    ...
    if isinstance(value, SymbolicValue):
        symbolic_name = _canonicalize_symbolic_name(value.name)
...
def _is_generator_close_completion_exception(state: VMState, exc_name: str) -> bool:
    """Return whether CPython ``generator.close()`` will swallow this raise."""
    if exc_name not in {"GeneratorExit", "StopIteration"} or not state.call_stack:
        return False
    frame = state.call_stack[-1]
    if frame.protocol_method != "__generator_resume__":
        return False
    request = frame.protocol_retained_operand
    return getattr(request, "operation", None) == "close"
...
def _extract_named_exception(value: object) -> str | None:
    ...
    if isinstance(type_name, str) and ("Error" in type_name or "Exception" in type_name):
        return _canonicalize_symbolic_name(type_name)
    if not hasattr(value, "__dict__"):
        return None
    maybe_name = value.__dict__.get("name")
    if isinstance(maybe_name, str) and ("Error" in maybe_name or "Exception" in maybe_name):
        return _canonicalize_symbolic_name(maybe_name)
    return None


def _issue_kind_for_exception(value: object, exc_name: str) -> IssueKind:
    """Return a specific runtime issue kind for supported explicit exception raises."""
    del value
    return issue_kind_for_explicit_raise(exc_name)
...
def _canonicalize_symbolic_name(name: str) -> str:
