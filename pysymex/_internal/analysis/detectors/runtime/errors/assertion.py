...
def _has_recent_assertion_opcode(state: VMState, lookback_window: int) -> bool:
    ...
    if state.pc <= 0:
        return False
    start = max(0, state.pc - lookback_window)
...
class AssertionErrorDetector(Detector):
    ...
    def _looks_like_assertion_marker(value: object) -> bool:
        """Return True when a stack value clearly represents AssertionError."""
        return _looks_like_assertion_marker(value)

    def _has_recent_assertion_opcode(self, state: VMState) -> bool:
        """Return True when nearby bytecode constructs an AssertionError."""
        return _has_recent_assertion_opcode(state, self.LOOKBACK_WINDOW)
    ...
    def check(
        ...
        """
        return _check_assertion_error(self, state, instruction, _solver_check)
...
def _check_assertion_error(
    ...
    """Inspect one raise instruction for assertion-error evidence."""
    ...
    for offset in range(1, min(3, len(state.stack)) + 1):
        if _looks_like_assertion_marker(state.stack[-offset]):
            is_assertion = True
            break
    if not is_assertion:
        is_assertion = _has_recent_assertion_opcode(state, detector.LOOKBACK_WINDOW)

    if not is_assertion:
        return None

    constraints = list(state.path_constraints)
    model_result = get_model_if_satisfiable_result(constraints, solver_check)
