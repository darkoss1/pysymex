...
if TYPE_CHECKING:
    ...
    from pysymex._internal.analysis.detectors.filter.types import FilterIssueLike, TIssue
...
def _has_trigger_evidence(issue: FilterIssueLike) -> bool:
...
def _get_issue_rank(issue: FilterIssueLike) -> tuple[int, int, float, int]:
...
def deduplicate_issues(issues: Sequence[TIssue]) -> list[TIssue]:
    ...
    for issue in merged_variants:
        ...
        if family_key not in family_buckets or _get_issue_rank(issue) > _get_issue_rank(
    ...
    name_error_family_lines = {
        (issue.line_number, getattr(issue, "function_name", None))
        for issue in deduped
        if _get_kind_str(issue.kind) in {"NAME_ERROR", "UNBOUND_VARIABLE"}
    }
    if name_error_family_lines:
        deduped = [
            issue
            for issue in deduped
            if _get_kind_str(issue.kind) != "UNHANDLED_EXCEPTION"
            or (issue.line_number, getattr(issue, "function_name", None))
            not in name_error_family_lines
            or _unhandled_exception_name(issue.message) not in {"NameError", "UnboundLocalError"}
        ]
...
def _unhandled_exception_name(message: str) -> str | None:
    """Return the exception class named by an ``UNHANDLED_EXCEPTION`` message."""
    prefix = "Path raises unhandled exception: "
    if not message.startswith(prefix):
        return None
    candidate = message.removeprefix(prefix).split(":", 1)[0].strip()
