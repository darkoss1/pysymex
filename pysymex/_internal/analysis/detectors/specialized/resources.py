...
Required Evidence:
    A method load on a variable previously registered in the VMState.freed_vars set.
...
from pysymex._internal.analysis.detectors.calls import extract_argc, get_call_target_name
...
def _resolve_receiver_name(state: VMState, argc: int, target_name: str) -> str | None:
    ...
    for index in candidate_indices:
        ...
        candidate_name = SpecializedDetectorOps.display_name(candidate)
...
class UseAfterFreeDetector(Detector):
    ...
    relevant_opcodes = DIRECT_CALL_OPCODES | frozenset(("LOAD_METHOD", "LOAD_ATTR"))
    ...
    def check(
        ...
        if instruction.opname in DIRECT_CALL_OPCODES:
            ...
            resolved_target = SpecializedDetectorOps.target_name(state, argc)
        ...
        elif _is_method_attribute_load(instruction):
            if state.stack:
                top = state.peek()
                top_name = SpecializedDetectorOps.display_name(top)
...
def _is_method_attribute_load(instruction: dis.Instruction) -> bool:
    """Return whether *instruction* loads a method rather than a plain attribute."""
    if instruction.opname == "LOAD_METHOD":
        return True
    if instruction.opname != "LOAD_ATTR":
        return False
    if "NULL|self" in str(instruction.argrepr):
        return True
    if isinstance(instruction.arg, int):
        return bool(instruction.arg & 1)
    return False
