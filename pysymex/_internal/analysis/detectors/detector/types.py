...
class Severity(Enum):
    ...
    levels. ``ERROR`` / ``WARNING`` / ``INFO`` support contract and diagnostic
    reporting surfaces that use message-oriented levels.
    """
...
def _format_issue(issue: Issue) -> str:
    """Format an issue without expanding the public dataclass body."""
    if issue.severity is not None:
        sev = issue.severity.name.lower()
        kind = issue.kind.name.replace("_", " ").lower()
        loc = f"{issue.file}:{issue.line}"
        if issue.column:
            loc += f":{issue.column}"
        conf = f" ({issue.confidence:.0%} confident)" if issue.confidence < 1.0 else ""
        return f"[{sev}] {kind} at {loc}{conf}: {issue.message}"

    lines = [f"[{issue.kind.name}] {issue.message}"]
    if issue.filename or issue.line_number or issue.function_name:
        location: list[str] = []
        if issue.filename:
            location.append(issue.filename)
        if issue.function_name:
            location.append(f"in {issue.function_name}()")
        if issue.line_number:
            location.append(f"line {issue.line_number}")
        lines.append(f"  Location: {', '.join(location)}")
    if issue.pc:
        lines.append(f"  PC: {issue.pc}")
...
def _issue_to_dict(issue: Issue) -> dict[str, object]:
    """Serialise an issue with the stable detector report schema."""
    return {
