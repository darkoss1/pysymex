...
def _is_safe(module_name: str) -> bool:
    ...
    for node in tree.body:
        if isinstance(node, ast.Import):
            for alias in node.names:
                if not StdlibImports.is_safe(alias.name):
                    continue
                try:
                    module = importlib.import_module(alias.name)
                except (ImportError, AttributeError, TypeError, ValueError):
                    logger.debug(
                        "Skipped compile-only stdlib import: %s",
                        alias.name,
                        exc_info=True,
                    )
                    continue
            ...
            if node.module is None or not StdlibImports.is_safe(node.module):
                continue
            try:
                module = importlib.import_module(node.module)
            except (ImportError, AttributeError, TypeError, ValueError):
                logger.debug(
                    "Skipped compile-only stdlib from-import: %s",
                    node.module,
                    exc_info=True,
                )
                continue
...
class StdlibImports:
    ...
    root = staticmethod(_root)
    is_safe = staticmethod(_is_safe)
    populate_concrete = staticmethod(_populate_concrete)
