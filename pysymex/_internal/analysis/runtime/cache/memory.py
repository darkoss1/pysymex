...
from pysymex._internal.analysis.runtime.cache.keying import cache_hit_rate
...
class LRUCache(Generic[K, V]):
    """Thread-safe OrderedDict-backed LRU cache with pickling support."""

    def __init__(self, maxsize: int = 1000) -> None:
        """Initialize a thread-safe LRU cache with a maximum capacity."""
        if maxsize < 0:
            msg = "LRU cache maxsize must be non-negative"
    ...
    def lookup(self, key: K) -> tuple[bool, V | None]:
        """Return hit status and value while updating order and counters."""
        with self.lock:
    ...
    def hit_rate(self) -> float:
        """Return the cache hit ratio from 0.0 to 1.0."""
        return cache_hit_rate(self.hits, self.misses)

    def __getstate__(self) -> dict[str, object]:
        """Return pickle state with the active thread lock stripped."""
        state = self.__dict__.copy()
        state["lock"] = None
        return state

    def __setstate__(self, state: dict[str, object]) -> None:
        """Restore pickle state with a fresh reentrant lock."""
