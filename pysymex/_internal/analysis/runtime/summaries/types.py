...
class PreconditionCheckStatus(Enum):
    """Outcome of checking instantiated summary preconditions."""
...
class PreconditionCheckResult:
    """Structured precondition-check result preserving solver inconclusiveness."""

    status: PreconditionCheckStatus
    counterexample: dict[str, object] | None = None
    reason: str | None = None

    @property
    def is_satisfied(self) -> bool:
        """Return true only when preconditions are definitely satisfied."""
        return self.status == PreconditionCheckStatus.SATISFIED
...
@dataclass(frozen=True, slots=True)
class SummaryExceptionInfo:
    """Information about an exception that a function summary may raise."""
...
class _FunctionSummaryState:
    name: str
    qualname: str
    module: str
    parameters: list[ParameterInfo]
    return_type: str | None
    preconditions: list[z3.BoolRef]
    postconditions: list[z3.BoolRef]
    modified: list[ModifiedVariable]
    reads: list[ReadVariable]
    calls: list[CallSite]
    may_raise: list[SummaryExceptionInfo]
    is_pure: bool
    is_deterministic: bool
    is_recursive: bool
    complexity: str
    return_constraint: z3.BoolRef | None
    return_var: z3.ExprRef | None


class _FunctionSummaryParameterMixin(_FunctionSummaryState):
...
class _FunctionSummaryMutationMixin(_FunctionSummaryState):
    ...
    def add_exception(self, exc: SummaryExceptionInfo) -> None:
        """Add a potential exception."""
        self.may_raise.append(exc)


class _FunctionSummaryQueryMixin(_FunctionSummaryState):
...
class _FunctionSummaryCloneMixin(_FunctionSummaryState):
...
class FunctionSummary(
    _FunctionSummaryParameterMixin,
    _FunctionSummaryMutationMixin,
    _FunctionSummaryQueryMixin,
    _FunctionSummaryCloneMixin,
):
    ...
    may_raise: list[SummaryExceptionInfo] = field(default_factory=list[SummaryExceptionInfo])
    is_pure: bool = False
