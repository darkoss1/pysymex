...
Used by :mod:`pysymex._internal.execution.opcodes.common.numeric.ops` when exponent or shift
amounts are symbolic: ``extract_interval`` walks linear comparisons on one Z3
integer symbol, then ``piecewise_exact_*`` encodes finite non-negative ranges
without widening to full havoc.
...
class IntInterval:
    ...
    def has_finite_non_negative_range(self) -> bool:
        """Return whether both bounds prove a finite non-negative interval."""
...
def piecewise_exact_power(
    ...
    for value in reversed(values[:-1]):
        tail = z3.If(exponent == ConstraintValues.int(value), exact_power_expr(base, value), tail)
    return tail
...
def piecewise_exact_shift_left(
    ...
    values = list(range(lower, upper + 1))
    tail = value * ConstraintValues.int(1 << values[-1])
    for current in reversed(values[:-1]):
        tail = z3.If(
            shift == ConstraintValues.int(current),
            value * ConstraintValues.int(1 << current),
            tail,
        )
    return tail
...
def piecewise_exact_shift_right(
    ...
    values = list(range(lower, upper + 1))
    tail = ScalarValueOps.py_floor_div(value, ConstraintValues.int(1 << values[-1]))
    for current in reversed(values[:-1]):
        tail = z3.If(
            shift == ConstraintValues.int(current),
            ScalarValueOps.py_floor_div(value, ConstraintValues.int(1 << current)),
            tail,
        )
    return tail
