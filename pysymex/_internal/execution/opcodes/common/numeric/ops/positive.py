...
from pysymex._internal.core.types.affinity import NUMERIC_AFFINITIES
...
from pysymex._internal.execution.opcodes.common.numeric.fallbacks import (
    unary_positive_uncertain_event,
)
from pysymex._internal.execution.opcodes.common.numeric.labels import UNARY_POSITIVE_TYPE_UNCERTAIN
...
_NUMERIC_UNARY_AFFINITIES = NUMERIC_AFFINITIES
...
def handle_precise_unary_positive(
...
def continue_precise_unary_positive_value(
    instr: dis.Instruction,
    state: VMState,
    ...
    invalid_type = invalid_unary_positive_type_name(value)
...
def invalid_unary_positive_type_name(value: object) -> str | None:
    ...
    symbolic_type_name = _INVALID_UNARY_AFFINITY_TYPES.get(symbolic_affinity(value))
    if symbolic_type_name is not None:
        return symbolic_type_name
