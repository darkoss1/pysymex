...
Bridges bytecode call setup to :mod:`pysymex._internal.execution.calls.model.dispatch`
...
from pysymex._internal.execution.calls.object.maps import is_object_map, map_get, map_to_stack_dict
...
def handle_common_make_function(
    ...
    closure = None
    annotations = None
    if flags & 0x08 and state.stack:
        closure = state.pop()
    if flags & 0x04 and state.stack:
        annotations = state.pop()
    if flags & 0x02 and state.stack:
        kwdefaults = state.pop()
    if flags & 0x01 and state.stack:
        defaults = state.pop()
...
def handle_common_import_name(
    ...
    materialized = materialize_module(full_name, state)
    if materialized is None:  # Generic materialization is intentionally total.
        msg = f"failed to materialize module {full_name!r}"
        raise RuntimeError(msg)
    module_val, state = materialized
...
def handle_common_precall(
    ...
    """Handle PRECALL call-site metadata (Python 3.11)."""
