...
def handle_common_load_super_attr(
    ...
    """
    require_call_stack_depth(state, instr, 3, "LOAD_SUPER_ATTR operands")
    receiver = state.pop()
    state.pop()
    state.pop()
...
def handle_common_load_super_variants(
    ...
    _ = ctx
    require_call_stack_depth(state, instr, 2, "super opcode operands")
    state.pop()
    state.pop()
    attr_name = str(instr.argval)
    return _unsupported_super_result(
    ...
    for index, cls in enumerate(modeled_cls.mro):
        for method in cls.methods.values():
            ...
            if isinstance(method_code, types.CodeType) and (
                list(get_instructions(method_code)) == active_instructions
            ):
                return index
    return None
