...
from pysymex._internal.core.effects.locations import closure_write_location
...
def handle_common_load_deref(
    ...
    if not (isinstance(cell, SymbolicObject) and cell.name.startswith("cell_")):
        ...
        cell = SymbolicObject(f"cell_{name}", addr, ConstraintValues.int(addr), {addr})
...
def handle_common_store_deref(
    ...
    name = str(instr.argval)
    LocalStackOps.require_depth(state, instr, 1, "STORE_DEREF value")
    value = state.pop()
...
def handle_common_cell_ops(
    ...
    if instr.opname == "MAKE_CELL":
        ...
        if val is None:
            cell_value: StackValue = SymbolicNoneType()
        elif is_bound(val):
            cell_value = val
        else:
            cell_value = SymbolicValue.from_const(val)

        addr = next_address()
        state = state.store_heap(addr, cell_value)
        cell_obj = SymbolicObject(f"cell_{name}", addr, ConstraintValues.int(addr), {addr})
...
def handle_common_load_from_dict_or_deref(
    ...
    name = str(instr.argval)
    LocalStackOps.require_depth(state, instr, 1, "LOAD_FROM_DICT_OR_DEREF namespace")
    state.pop()
...
def handle_common_load_from_dict_or_globals(
    ...
    name = str(instr.argval)
    LocalStackOps.require_depth(state, instr, 1, "LOAD_FROM_DICT_OR_GLOBALS namespace")
    state.pop()
...
def handle_common_load_closure(
    ...
    if not (isinstance(val, SymbolicObject) and val.name.startswith("cell_")):
        ...
        if is_bound(val) and val is not None:
            ...
            state = state.add_constraint(constraint)
            state = state.store_heap(addr, sym_val)
        val = SymbolicObject(f"cell_{name}", addr, ConstraintValues.int(addr), {addr})
