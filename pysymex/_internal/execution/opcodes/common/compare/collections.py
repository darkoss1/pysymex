...
_ExactSequenceKind = Literal["list", "tuple", "bytes", "bytearray"]
...
def _sequence_kinds_can_compare_equal(
    ...
    if left_kind in {"list", "tuple"} or right_kind in {"list", "tuple"}:
...
def _exact_item_equal(left: object, right: object) -> bool | None:
    exact_equal = exact_concrete_equal(left, right)
    if exact_equal is not None:
        return exact_equal
