...
class CallLowerer:
    ...
    def resolve_layout(
        ...
        *,
        ...
        if receiver_is_argument or (
            not isinstance(receiver_or_null, SymbolicNoneType) and receiver_or_null is not None
        ):
            final_args.insert(0, receiver_or_null)

        return CallLayout(func_obj=func_obj, args=final_args, kwargs=kwargs)
    ...
    def is_likely_callable(self, obj: object) -> bool:
        ...
        if getattr(obj, "_pysymex_bound_type_call", False) is True:
            return True
        if isinstance(obj, SymbolicMethod):
            return True
        if callable(obj):
            return True
    ...
    def emit_none_check(self, func_obj: StackValue) -> z3.BoolRef:
        ...
        if isinstance(func_obj, (SymbolicNoneType, SymbolicValue)):
            return Z3_TRUE if isinstance(func_obj, SymbolicNoneType) else func_obj.is_none
