...
from pysymex._internal.models.builtins.types.containers.bytes.bytearray.exact import (
    retained_byte_items,
)
...
def exact_slice_replacement_items(
    ...
    resolved = SymbolicObject.resolve_runtime_container(value, state)
    if _is_bytearray_target(target):
        raw_items = concrete_iterable_items(resolved, state)
        byte_items = retained_byte_items(raw_items) if raw_items is not None else None
        return cast("list[StackValue] | None", byte_items)

    sequence = ConcreteExtractionPolicy.sequence(resolved)
...
def concrete_iterable_items(value: object, state: VMState) -> list[StackValue] | None:
    """Return exact iterable items when the source can be consumed without executing user code."""
