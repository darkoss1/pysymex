...
:mod:`pysymex._internal.execution.opcodes.common.control.feasibility`.
...
def complete_retained_membership(
    ...
    if return_value is None:
        return_value = SymbolicNoneType("membership_None")
...
def complete_retained_truth_jump(
    ...
    if retained_operand is None or instructions is None:
        msg = "Missing retained truth-jump continuation state"
        raise VMStateError(msg)
    if frame.return_pc < 0 or frame.return_pc >= len(instructions):
        msg = "Truth-jump continuation return PC is outside caller instructions"
        raise VMStateError(msg)
    ...
    if not isinstance(instr, dis.Instruction) or instr.opname not in {
        ...
        msg = "Truth-jump continuation did not resume a short-circuit jump"
        raise VMStateError(msg)
