...
"""Stack manipulation opcode wrappers for Python 3.11.

Registers ``POP_TOP``, ``COPY``, ``SWAP``, ``PUSH_NULL``, ``EXTENDED_ARG``,
and ``CACHE`` via :mod:`pysymex._internal.execution.opcodes.common.stack`. Does not
manage frame locals, the block stack, or operand decoding beyond ``dis``.
.
...
handle_pop_top = handle_py311_pop_top
handle_copy = handle_py311_copy
handle_swap = handle_py311_swap
handle_extended_arg = handle_py311_extended_arg
handle_push_null = handle_py311_push_null
handle_cache = handle_py311_cache
