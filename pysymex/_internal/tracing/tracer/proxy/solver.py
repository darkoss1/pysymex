...
from pysymex._internal.tracing.tracer.proxy.delegation import SolverDelegationMixin
from pysymex._internal.tracing.tracer.proxy.telemetry import (
    SolverTelemetrySink,
    TraceStateGetter,
    cache_hits_for,
...
def _check_result_name(result: SolverResult | z3.CheckSatResult) -> str:
    """Return trace schema text for raw-Z3 or structured solver check results."""
    if isinstance(result, SolverResult):
        return solver_result_name(result)
    return z3_check_result_name(result)


def _check_result_model(result: SolverResult | z3.CheckSatResult) -> object:
    """Return a model excerpt source only for structured SAT check results."""
    if isinstance(result, SolverResult):
        return result.model
    return None
...
class TracingSolverProxy(SolverDelegationMixin):
    """Transparent proxy around :class:`~pysymex._internal.core.solver.IncrementalSolver`.
    ...
    Args:
        inner:        The real :class:`~pysymex._internal.core.solver.IncrementalSolver`.
        tracer:       The :class:`ExecutionTracer` that receives ``on_solve``
                      notifications.
        state_getter: Zero-argument callable that returns the *current*
                      :class:`~pysymex._internal.core.state.record.VMState`, used to embed
    ...
    def check(
        ...
        *assumptions: z3.BoolRef,
        ...
        return self._with_solver_telemetry(
            ...
            result_name=_check_result_name,
            model_for_result=_check_result_model,
            method_name="check",
        )
