...
from pysymex._internal.analysis.records import iter_trigger_assignments
from pysymex._internal.config.defaults import VERSION
from pysymex._internal.reporting.formatters.base import Formatter
from pysymex._internal.reporting.formatters.rich.verify import RichFormatterExtraMixin
        ...
        try:
            ...
            if result.issues:
                ...
                for issue in result.issues:
                    ...
                    severity_str = severity.name.upper() if severity else ""
                    ...
                    crash_details = (
                        f"[bold {color}]Location:[/bold {color}] {result.source_file}:{issue.line_number} in {result.function_name}()\n"
                        f"[bold {color}]Type:[/bold {color}]    {issue.kind.name}\n"
                        f"[bold {color}]Error:[/bold {color}]    {issue.message}"
                    )
                    ...
                    crash_panel = Panel(
                        crash_details,
                        title=f"[bold {color}][ {issue.kind.name} ][/bold {color}]",
                        title_align="left",
                        border_style=color,
                        box=box.ROUNDED,
                        padding=(0, 2),
