...
from pysymex._internal.cli.formatters.base import (
    format_verify_issue,
    verify_issue_count,
...
class VerifyTextMixin:
    ...
    def _format_verify_ascii(self, results: Sequence[Any], total: int, duration: float) -> str:
        ...
        for result in results:
            ...
            inferred_properties = data["inferred_properties"]
            if isinstance(inferred_properties, list) and inferred_properties:
                lines.append("      Inferred properties:")
                for prop in cast("list[dict[str, object]]", inferred_properties):
                    lines.append(f"        - {_format_inferred_property_row(prop)}")
    ...
    def _format_verify_rich(self, results: Sequence[Any], total: int, duration: float) -> str:
        ...
        console.print(
            Panel(
                "[bold cyan]pysymex contract verification[/bold cyan]",
                border_style="cyan",
                box=box.ROUNDED,
                expand=False,
            )
...
def _format_inferred_property_row(prop: dict[str, object]) -> str:
    """Format one compact ASCII inferred-property row."""
    proof = prop.get("proof")
    status = "UNKNOWN"
    if isinstance(proof, dict):
        proof_payload = cast("dict[str, object]", proof)
        status = str(proof_payload.get("status", "UNKNOWN"))
    return f"[{status}] {prop['kind']}: {prop['description']}"
