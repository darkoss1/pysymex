...
_process_caches_disabled_fast = False
...
def is_process_cache_disabled() -> bool:
    ...
    return _process_caches_disabled_fast
...
def register_process_cache_clearer(name: str, clearer: ProcessCacheClearer) -> None:
    ...
    """
    if not name:
        msg = "cache clearer name must be non-empty"
...
def process_caches_disabled(enabled: bool = True) -> Generator[None]:
    ...
    """
    global _process_caches_disabled_fast
    if not enabled:
        yield
        return
    ...
    old_fast = _process_caches_disabled_fast
    _process_caches_disabled_fast = True
    try:
        yield
    finally:
        _PROCESS_CACHES_DISABLED.reset(token)
        _process_caches_disabled_fast = old_fast
        clear_process_caches()
