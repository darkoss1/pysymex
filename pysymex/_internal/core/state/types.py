...
class CallFrame:
    ...
    has_contract_frame: bool = False
    argument_aliases: tuple[tuple[str, StackValue], ...] = ()
    caller_offset: int | None = None
    write_event_start_index: int = 0
    function_code: object | None = None
    ...
    def hash_value(self) -> int:
        ...
        """
        frame_hash = (
            hash((self.function_name, id(self.function_code), self.return_pc, self.stack_depth))
    ...
    @classmethod
    def fork_stack(cls, call_stack: list[CallFrame]) -> list[CallFrame]:
        """Return a fork-safe call-stack copy for a child VM state."""
        if not call_stack:
            return []
        if not any(frame.summary_builder is not None for frame in call_stack):
            return list(call_stack)

        return [
            cls(
                function_name=frame.function_name,
                return_pc=frame.return_pc,
                local_vars=frame.local_vars.cow_fork(),
                stack_depth=frame.stack_depth,
                caller_stack=frame.caller_stack,
                caller_instructions=frame.caller_instructions,
                summary_builder=(
                    copy_summary_builder(frame.summary_builder)
                    if frame.summary_builder is not None
                    else None
                ),
                is_init_call=frame.is_init_call,
                init_instance=frame.init_instance,
                protocol_method=frame.protocol_method,
                protocol_retained_operand=frame.protocol_retained_operand,
                protocol_fallbacks=frame.protocol_fallbacks,
...
def copy_summary_builder(builder: object) -> object:
    ...
    if callable(builder_clone):
        try:
            ...
            msg = f"Failed to clone call-frame summary builder of type {type(builder).__qualname__}"
            raise VMStateError(
                msg,
            ) from exc

    msg = f"Call-frame summary builder of type {type(builder).__qualname__} must define clone()"
    raise VMStateError(
        msg,
    )
