...
class ValueBitwiseXorMixin:
    ...
    def __xor__(self: SymbolicValueSelf, other: object) -> SymbolicValueSelf:
        ...
        if self_affinity == "int" and other_affinity == "int":
            ...
            if z3.eq(left_bv, right_bv):
                res_bv = ConstraintValues.bitvec(0, BV_WIDTH)
            elif left_bv.decl().kind() == Z3_OP_BXOR and left_bv.num_args() == 2:
                a = left_bv.arg(0)
                b = left_bv.arg(1)
                if z3.eq(a, right_bv):
                    res_bv = b
            ...
            elif right_bv.decl().kind() == Z3_OP_BXOR and right_bv.num_args() == 2:
                a = right_bv.arg(0)
                b = right_bv.arg(1)
                if z3.eq(a, left_bv):
                    res_bv = b
    ...
    def __invert__(self: SymbolicValueSelf) -> SymbolicValueSelf:
        ...
        concrete = ScalarValueOps.apply_concrete_integral_unary_op(self, lambda a: ~a)
        if concrete is not None:
            return concrete
        self_affinity = self.affinity_type

        if self_affinity == "int":
            res_bv = ~ScalarValueOps.int_to_bv(self.z3_int)
            res_int = ScalarValueOps.bv_to_int(res_bv)
            return SymbolicValue(
                _name=f"(~{self.name})",
                z3_int=res_int,
                is_int=Z3_TRUE,
        ...
        res_bv = ~ScalarValueOps.int_to_bv(self.z3_int)
        res_int = ScalarValueOps.bv_to_int(res_bv)
        is_int_res = simplify_expr(z3.Or(self.is_int, self.is_bool))
