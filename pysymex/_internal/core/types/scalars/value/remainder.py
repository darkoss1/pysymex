...
from pysymex._internal.core.constants import Z3_FALSE, Z3_FLOAT_ONE, Z3_FLOAT_ZERO, Z3_TRUE, Z3_ZERO
...
class ValueRemainderMixin:
    ...
    def __mod__(self: SymbolicValueSelf, other: object) -> SymbolicValueSelf:
        ...
        Limitations:
            ...
            path.
        """
        if ScalarValueOps.is_concrete_val(self) and ScalarValueOps.is_concrete_val(other):
            try:
        ...
        if self_affinity == "int" and other_affinity == "int":
            ...
            safe_divisor = (
                ...
                else (ScalarValueOps.guarded_nonzero_divisor(other.z3_int))
            )
            raw_res = ScalarValueOps.py_mod(self.z3_int, safe_divisor)
            guarded_res = (
                raw_res
                if ScalarValueOps.concrete_numeric_is_nonzero(cv)
    ...
    def __floordiv__(self: SymbolicValueSelf, other: object) -> SymbolicValueSelf:
        ...
        if self_affinity == "int" and other_affinity == "int":
            ...
            raw_res = ScalarValueOps.py_floor_div(self.z3_int, safe_divisor)
            guarded_res = (
                raw_res
                if ScalarValueOps.concrete_numeric_is_nonzero(cv)
                else z3.If(other.z3_int != 0, raw_res, Z3_ZERO)
