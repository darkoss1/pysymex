...
from pysymex._internal.core.solver.engine.queries import check_sat_result, get_model
...
def _concrete_bound(
    ...
    if symbolic_parts is not None:
        ...
        if z3.is_true(is_none):
            return None
        if not z3.is_false(is_none) and z3.is_true(simplify_expr(is_none)):
            return None
        ...
        is_int = z3.is_true(is_int_expr) or (
            not z3.is_false(is_int_expr) and z3.is_true(simplify_expr(is_int_expr))
        )
