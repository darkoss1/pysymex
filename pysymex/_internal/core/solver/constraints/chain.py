...
from pysymex._internal.core.solver.constraints.theory import is_bitvector_smt_theory
...
class ConstraintChain:
    ...
    __slots__ = (
        ...
        "constraint",
        "parent",
        "_has_sequence_theory",
        "_has_excluded_theory",
    )
    ...
    def __init__(
        ...
        self._cached_constraints: tuple[z3.BoolRef, ...] | None = None
        ...
        if parent is None:
            ...
            if constraint is not None:
                from pysymex._internal.core.solver.formula import formula_meta

                meta = formula_meta(constraint)
                self._has_sequence_theory = meta.contains_sequence
                self._has_excluded_theory = bool(
                ...
                self._has_sequence_theory = False
                self._has_excluded_theory = False
            ...
            if constraint is not None and self._constraint_hash is not None:
                ...
                meta = formula_meta(constraint)
                self._has_sequence_theory = parent._has_sequence_theory or meta.contains_sequence
                self._has_excluded_theory = parent._has_excluded_theory or bool(
                    meta.contains_bitvector
                    or meta.contains_float
                    or meta.contains_array
                    or meta.contains_modulo
                ...
                self._has_sequence_theory = parent._has_sequence_theory
                self._has_excluded_theory = parent._has_excluded_theory
    ...
    def to_list(self) -> list[z3.BoolRef]:
        """Materialize the chain as a Python list. O(n) once per immutable head.

        Returns constraints in chronological order (oldest first). A tuple cache is
        kept internally so repeated feasibility/detector checks on the same
        persistent chain head do not walk all ancestors again. A fresh list is
        returned to preserve the public mutable-list contract.
        """
        cached = self._cached_constraints
        ...
        while node is not None and node.constraint is not None:
            ...
            node = node.parent
        result.reverse()
        self._cached_constraints = tuple(result)
        return result
    ...
    def has_sequence_theory(self) -> bool:
        """Return whether any recorded constraint contains sequence SMT terms."""
        return self._has_sequence_theory

    def has_excluded_theory(self) -> bool:
        """Return whether any recorded constraint contains excluded (complex) SMT terms."""
        return self._has_excluded_theory
