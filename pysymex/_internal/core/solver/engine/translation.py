...
from pysymex._internal.core.solver.engine.constraints import as_bool_constraint
...
class SolverTranslationMixin(SolverMixinContract):
        ...
        while stack:
            ...
            try:
                ...
                if logger.state.debug_enabled:
                    logger.debug(
                        "Failed to read Z3 AST sort during translation signature",
                        exc_info=True,
                    )
                sort_kinds.append(-1)
    ...
    def _translate_bool_constraint(self, constraint: z3.BoolRef) -> z3.BoolRef:
        ...
        try:
            translate = getattr(constraint, "translate", None)
            if not callable(translate):
                logger.warning("Z3 AST cannot be translated in this runtime")
                return constraint
            raw_translated = translate(target_ctx)
            ...
            logger.warning(
                "Z3 AST translation failed; retaining original constraint",
                exc_info=True,
            )
            return constraint
