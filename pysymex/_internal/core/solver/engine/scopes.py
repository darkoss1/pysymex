...
from pysymex._internal.core.solver.engine.configuration import rlimit_for_timeout_ms
from pysymex._internal.core.solver.engine.events import emit_solver_event
...
class SolverScopeMixin(SolverMixinContract):
    ...
    def constraint_optimizer(self) -> IndependenceOptimizer:
    ...
    def check(self, *assumptions: z3.BoolRef, need_model: bool = False) -> SolverResult:
        ...
        if effective_timeout_ms <= 0:
            emit_solver_event(EventType.SOLVER_UNKNOWN, 1.0)
            ...
            if check_cache_key in self._check_cache:
                ...
                if cached is not None:
                    if cached.is_sat:
                        ...
                        emit_solver_event(EventType.SOLVER_SAT, 1.0)
                    elif cached.is_unsat:
                        self._unsat_result_count += 1
                        emit_solver_event(EventType.SOLVER_UNSAT, 1.0)
                    else:
                        self._unknown_result_count += 1
                        emit_solver_event(EventType.SOLVER_UNKNOWN, 1.0)
                    return cached
        ...
        try:
            ...
            self._z3_check_count += 1
            emit_solver_event(EventType.SOLVER_QUERY, 0.0, {"clauses": num_clauses})
