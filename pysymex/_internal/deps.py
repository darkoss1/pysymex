...
if TYPE_CHECKING:
    ...
    class Z3Diagnostics(TypedDict):
        """Diagnostic metadata for the Z3 SMT solver dependency.

        Attributes:
            module_repr (str): Representation string of the loaded Z3 module.
            module_path (str | None): File path of the loaded Z3 module if available, otherwise None.
            z3_version (str | None): Package version of the installed Z3 package if available, otherwise None.
            z3_solver_version (str | None): Internal version of the Z3 solver engine if available, otherwise None.
            has_required_api (dict[str, bool]): Mapping from required Z3 API function names to availability status.
            missing_api (list[str]): List of required Z3 API functions that are not present in the module.
...
def is_z3_ready(module: ModuleType) -> bool:
...
def z3_diagnostics(module: ModuleType | None = None) -> Z3Diagnostics:
    ...
    return {
        ...
        "z3_version": package_version("z3"),
        "z3_solver_version": package_version("z3-solver"),
        "has_required_api": has_api,
        "missing_api": missing_api,
    }


def build_z3_error() -> RuntimeError:
    """Build z3 error."""
    diag = z3_diagnostics()
    details = [
...
def ensure_z3_ready(force_recheck: bool = False) -> ModuleType:
    ...
    if _cached_z3 is not None and not force_recheck and is_z3_ready(_cached_z3):
        return _cached_z3
    ...
    with _z3_lock:
        ...
        try:
            ...
            raise build_z3_error() from exc

        if is_z3_ready(z3_module):
            _cached_z3 = z3_module
            return z3_module

        raise build_z3_error()
