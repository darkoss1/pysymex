    ...
    for line in stdout_text.splitlines():
        if line.startswith(marker):
            ...
            try:
                ...
                parsed = SandboxSchema.mapping(loaded)
            except json.JSONDecodeError:
                parsed = None
            status = PayloadParseStatus.MALFORMED if parsed is None else PayloadParseStatus.OK
            continue
...
def validate_worker_envelope(parsed: dict[str, object]) -> None:
    ...
    if not isinstance(ok, bool):
        msg = "Sandbox worker envelope field 'ok' must be boolean"
