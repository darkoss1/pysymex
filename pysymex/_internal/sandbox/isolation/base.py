...
from pysymex._internal.sandbox.paths import jail_relative_path, validate_sandbox_filename
...
class IsolationBackend(ABC):
    ...
    def _create_jail(self) -> Path:
        ...
        return Path(
            tempfile.mkdtemp(
                prefix=f"pysymex_sandbox_{time.time_ns()}_",
                dir=base_dir,
            )
        )
    ...
    def _populate_jail(
        ...
        validate_sandbox_filename(filename, context="target")
        target = jail_relative_path(jail_root, filename, context="target")
        target.write_bytes(code)

        for rel_path, content in extra_files.items():
            out = jail_relative_path(jail_root, rel_path, context="extra_files")
            out_abs = out.resolve()
    ...
    def setup(self) -> None:
        ...
        try:
            ...
            self._is_setup = True
            logger.verbose("Sandbox jail created for %s at %s", self.name, self._jail_path)
        except Exception as exc:
            from pysymex._internal.sandbox.errors import SandboxSetupError

            logger.warning("Failed to create sandbox jail for %s", self.name, exc_info=True)
            self.cleanup()
    ...
    def cleanup(self) -> None:
        ...
        if self._process is not None:
            try:
                ...
                logger.warning(
                    "Failed to terminate sandbox process for %s",
                    self.name,
                    exc_info=True,
                )
            self._process = None
        ...
        self._destroy_jail()
        logger.verbose("Sandbox jail cleaned for %s", self.name)
