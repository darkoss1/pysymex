...
class MathIsfiniteModel(FunctionModel):
    ...
    def apply(
        ...
        if not args:
            result, constraint = SymbolicValue.symbolic_bool(f"isfinite_{state.pc}")
            return ModelResult(
                value=result,
...
class MathIsCloseModel(FunctionModel):
    ...
    def apply(
        ...
        if len(args) >= 2:
            ...
            diff = z3.If(
                z3.fpGEQ(a_fp, b_fp),
                z3.fpSub(z3.RNE(), a_fp, b_fp),
                z3.fpSub(z3.RNE(), b_fp, a_fp),
            )
...
class MathIsinfModel(FunctionModel):
    ...
    def apply(
        ...
        if not args:
            result, constraint = SymbolicValue.symbolic_bool(f"isinf_{state.pc}")
            return ModelResult(
                value=result,
                constraints=[constraint],
                side_effects={
...
class MathIsnanModel(FunctionModel):
    ...
    def apply(
        ...
        if not args:
            result, constraint = SymbolicValue.symbolic_bool(f"isnan_{state.pc}")
            return ModelResult(
                value=result,
                constraints=[constraint],
                side_effects={
