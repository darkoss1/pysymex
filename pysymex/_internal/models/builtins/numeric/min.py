    ...
    def apply(
        ...
        if set(kwargs) - {"key", "default"} or (len(args) > 1 and "default" in kwargs):
            ...
            return ModelResult(
                ...
                side_effects=SideEffects.type_error(
                    "builtins.min",
                    "min() received invalid keyword arguments",
                ),
            )
        ...
        if sequence is not None:
            ...
            symbolic_min = symbolic_extreme_result(seq, f"min_{state.pc}", "min")
            if symbolic_min is not None:
                return model_result_with_side_effects(symbolic_min, iterator_side_effects)
            if all(
                not isinstance(x, (SymbolicValue, SymbolicType)) for x in seq
            ):  # x is object from list[object]
                concrete_min = BuiltinAggregatePolicy.safe_min_concrete(
        ...
        if len(args) >= 2:
            ...
            result, constraints = ModelResult.symbolic_int(f"min_{state.pc}")
