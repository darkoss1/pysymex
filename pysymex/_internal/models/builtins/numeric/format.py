...
def _possibly_integer_argument(value: StackValue) -> bool:
    """Return true when an argument can still take Python integer values.

    Some symbolic carriers start as broad havoc values with a live ``z3_int``
    projection but without a simplified ``is_int == True`` tag.  ``bin(x)`` is
    still a useful integer constraint in that state: downstream string-count
    models can use the retained integer source to prove cases such as
    ``bin(x).count("1") == 0``.  Only reject arguments that are already known
    to be incompatible with Python's integer protocol.
    """
    return not _definite_invalid_integer_argument(value)
...
class DivmodModel(FunctionModel):
    ...
    def apply(
        ...
        if literal_a is not None and literal_b is not None:
            try:
                ...
                return ModelResult(
                    ...
                    constraints=[c1, c2],
                    side_effects=SideEffects.zero_division_error("builtins.divmod", str(exc)),
                )
            return ModelResult(value=(SymbolicValue.from_const(q), SymbolicValue.from_const(r)))
        definite_builtin_values = (
            int,
            float,
            bool,
            str,
        ...
        if (a is None or isinstance(a, definite_builtin_values)) and (
            b is None or isinstance(b, definite_builtin_values)
        ):
            quotient, c1 = SymbolicValue.symbolic(f"divmod_q_{state.pc}")
            remainder, c2 = SymbolicValue.symbolic(f"divmod_r_{state.pc}")
            return ModelResult(
                ...
                side_effects=SideEffects.type_error(
                    "builtins.divmod",
                    "divmod() operands do not support division",
                ),
            )
    ...
    def apply(
        ...
        if _definite_invalid_integer_argument(value):
            ...
            return ModelResult(
                ...
                side_effects=SideEffects.type_error(
                    "builtins.bin",
                    "bin() requires an integer argument",
                ),
            )
        ...
        if val is not None:
            ...
            if _possibly_integer_argument(value):
    ...
    def apply(
        ...
        if _definite_invalid_integer_argument(value):
            ...
            return ModelResult(
                ...
                side_effects=SideEffects.type_error(
                    "builtins.oct",
                    "oct() requires an integer argument",
                ),
            )
        result, constraint = SymbolicString.symbolic(f"oct_{state.pc}")
    ...
    def apply(
        ...
        if _definite_invalid_integer_argument(value):
            ...
            return ModelResult(
                ...
                side_effects=SideEffects.type_error(
                    "builtins.hex",
                    "hex() requires an integer argument",
                ),
            )
        result, constraint = SymbolicString.symbolic(f"hex_{state.pc}")
        constraints: list[z3.BoolRef] = [constraint]
