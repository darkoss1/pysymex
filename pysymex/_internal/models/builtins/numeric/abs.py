...
class AbsModel(FunctionModel):
    ...
    def apply(
        ...
        if isinstance(x, SymbolicValue):
            result, constraints = ModelResult.symbolic_int(f"abs_{x.name}")
        ...
        if x is None or isinstance(
            x,
            (str, bytes, SymbolicString, list, tuple, dict, set, frozenset),
        ):
            result, constraint = SymbolicValue.symbolic(f"abs_{state.pc}")
            return ModelResult(
                value=result,
                ...
                side_effects=SideEffects.type_error(
                    "builtins.abs",
                    "abs() argument does not define an absolute value",
                ),
            )
