...
from pysymex._internal.models.builtins.iteration.hashability import exact_dict_items_from_iterable
    ...
    def apply(
        ...
        if d is not None:
            ...
            if other is not None:
                ...
                source = SymbolicObject.resolve(other_arg, state)
            ...
            if kwargs:
                keyword_updates: dict[str, object] = dict(kwargs.items())
...
class DictSetdefaultModel(FunctionModel):
    ...
    def apply(
        ...
        side_effects: dict[str, object] = {}
        if d is not None and raw_key is not None:
            found, retained_value = d.concrete_value_for_key(raw_key)
            if found:
                result_value = (
            ...
            if presence is not None and z3.is_false(simplify_expr(presence)):
                updated_dict = d.__setitem__(raw_key, default_value)
                side_effects["dict_mutation"] = {
            ...
            selected_value = conditional_retained_lookup_value(
                ...
                default_value,
                state=state,
                name=f"{d.name}.setdefault({getattr(raw_key, 'name', raw_key)!s})",
            )
            if selected_value is not None:
                inserted_dict = d.__setitem__(raw_key, default_value)
                presence_check = (
                    presence
