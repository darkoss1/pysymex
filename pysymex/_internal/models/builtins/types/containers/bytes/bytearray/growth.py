...
from pysymex._internal.core.types.containers.sequence_precision import normalize_insert_index
    ...
    def apply(
        ...
        if b is not None:
            ...
            if byte_value is not None:
                b.z3_array = z3.Store(b.z3_array, old_len, ConstraintValues.int(byte_value))
                items = b.concrete_items
                if items is not None:
                    b.set_concrete_items([*items, byte_value])
            ...
            if extension is not None:
                for offset, byte_value in enumerate(extension):
                    b.z3_array = z3.Store(
                        b.z3_array,
                        old_len + offset,
                        ConstraintValues.int(byte_value),
                    )
                constraints.append(new_len == old_len + len(extension))
                items = b.concrete_items
                if items is not None:
                    b.set_concrete_items([*items, *extension])
    ...
    def apply(
        ...
        if b is not None:
            ...
            if byte_items is not None and index is not None and byte_value is not None:
                ...
                insert_index = normalize_insert_index(index, len(updated))
                if insert_index is not None:
                    updated.insert(insert_index, byte_value)
