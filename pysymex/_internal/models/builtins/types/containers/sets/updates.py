...
def _exact_set_source_items(
    ...
    for source_arg in sources:
        source = SymbolicObject.resolve(source_arg, state)
...
class SetIntersectUpdateModel(FunctionModel):
...
class SetDiffUpdateModel(FunctionModel):
...
class SetSymDiffUpdateModel(FunctionModel):
