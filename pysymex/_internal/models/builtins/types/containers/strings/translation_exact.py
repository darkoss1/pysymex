...
_UNKNOWN = object()
_TranslateValue = str | int | None
_TranslateTable = dict[int | str, _TranslateValue]
...
def exact_maketrans_result(args: list[StackValue], state: VMState) -> ModelResult | None:
    ...
    for arg in args:
        ...
        if not isinstance(value, (str, dict)):
            return ModelResult.method_type_error("str.maketrans", state)
...
def exact_translate_result(
    ...
    try:
        ...
        return ModelResult.method_type_error("str.translate", state)
    except ValueError as exc:
...
def _concrete_python_value(value: object, state: VMState) -> object:
    resolved = SymbolicObject.resolve(cast("StackValue", value), state)
    text = SymbolicString.concrete_literal(resolved)
    if text is not None:
        return text
    if isinstance(resolved, SymbolicValue):
        ...
        return _UNKNOWN
    if isinstance(resolved, SymbolicNoneType):
        return None
