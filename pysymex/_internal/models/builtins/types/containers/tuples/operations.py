...
class TupleAddModel(FunctionModel):
    ...
    def apply(
        ...
        other = TupleContainerOps.get_symbolic_tuple(args[1]) if len(args) > 1 else None
        if t is not None and other is not None:
            concatenated = concat_concrete_backed_sequences(t, other)
...
class TupleEqModel(FunctionModel):
    ...
    def apply(
        ...
        t = TupleContainerOps.get_symbolic_tuple(args[0]) if args else None
        other = TupleContainerOps.get_symbolic_tuple(args[1]) if len(args) > 1 else None
        result, constraint = SymbolicValue.symbolic_bool(f"tuple_eq_{state.pc}")
...
class TupleHashModel(FunctionModel):
    ...
    def apply(
        ...
        result, constraint = SymbolicValue.symbolic_int(f"tuple_hash_{state.pc}")
        return ModelResult(
            value=result,
            constraints=[constraint],
        )
