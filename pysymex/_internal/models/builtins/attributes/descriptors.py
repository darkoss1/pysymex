...
from pysymex._internal.core.calls.payload import MethodDescriptorPayload, function_payload
        ...
        if (
            ...
            return ModelResult(
                ...
                side_effects=SideEffects.type_error(
                    "builtins.property",
                    "property() received invalid arguments",
                ),
            )
...
class ClassmethodModel(FunctionModel):
    ...
    def apply(
        ...
        payload = function_payload(getattr(args[0], "_modeled_object", args[0]))
        if payload is not None:
            result, constraint = SymbolicValue.symbolic(f"classmethod_{state.pc}")
            result.attach_modeled_object(MethodDescriptorPayload(payload=payload, kind="class"))
            return ModelResult(value=result, constraints=[constraint])
        return ModelResult(value=args[0] if args else SymbolicNoneType())
...
class StaticmethodModel(FunctionModel):
    ...
    def apply(
        ...
        if payload is not None:
            result, constraint = SymbolicValue.symbolic(f"staticmethod_{state.pc}")
            result.attach_modeled_object(MethodDescriptorPayload(payload=payload, kind="static"))
            return ModelResult(value=result, constraints=[constraint])
        return ModelResult(value=args[0] if args else SymbolicNoneType())
    ...
    def apply(
        ...
        if len(args) > 1 or kwargs:
            return ModelResult(
                ...
                side_effects=SideEffects.type_error(
                    "builtins.vars",
                    "vars() accepts at most one argument",
                ),
            )
        if args and (
            args[0] is None
            ...
            return ModelResult(
                ...
                side_effects=SideEffects.type_error(
                    "builtins.vars",
                    "vars() argument must have a __dict__ attribute",
                ),
            )
        return ModelResult(value=result, constraints=[constraint])
...
class DirModel(FunctionModel):
    ...
    The zero-argument form preserves the exact currently bound local names.
    Object-specific ``__dir__`` dispatch remains abstract because invoking an
    arbitrary host hook would cross the symbolic execution boundary.
    """
    ...
    def apply(
        ...
        if not args:
            names = sorted(name for name, value in state.local_vars.items() if is_bound(value))
            return ModelResult(value=SymbolicList.from_const(names))
        return ModelResult(value=result, constraints=[constraint])
