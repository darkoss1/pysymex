...
from pysymex._internal.core.constants import Z3_TRUE, Z3_ZERO
    ...
    def to_constraint(self, var: z3.ExprRef | None) -> z3.BoolRef:
        ...
        if self.in_collection is not None:
            try:
                ...
                msg = "Unsupported quantified collection membership model"
...
class Quantifier:
    ...
    def to_z3(self) -> z3.BoolRef:
        ...
        if len(self.variables) != len(self.bounds):
            msg = "Each quantified variable requires exactly one bound"
        ...
        if self.kind == QuantifierKind.EXISTS:
            return z3.Exists(z3_vars, z3.And(bound_constraint, self.body))
        if self.kind == QuantifierKind.UNIQUE:
        ...
        if self.kind == QuantifierKind.COUNT:
            msg = "COUNT quantifier requires dedicated lowering and is unsupported here"
            raise ValueError(msg)
        msg = f"Unknown quantifier kind: {self.kind}"
        raise ValueError(msg)
