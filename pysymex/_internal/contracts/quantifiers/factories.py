...
Build :class:`~pysymex._internal.contracts.quantifiers.types.Quantifier` values for use inside string
...
def _condition_text(condition: object) -> str:
    ...
    if not isinstance(condition, str):
        msg = "Quantifier factory conditions must be string predicates"
        raise TypeError(msg)
    return condition
...
def forall(
    ...
    if result is None:
        msg = f"Failed to parse universal quantifier: {text}"
...
def exists(
    ...
    result = parser.parse(text)
    if result is None:
        msg = f"Failed to parse existential quantifier: {text}"
        raise ValueError(msg)
    return result
...
def exists_unique(
    ...
    result = parser.parse(text)
    if result is None:
        msg = f"Failed to parse unique existential quantifier: {text}"
        raise ValueError(msg)
    return result
