...
Maps decorated callables to :class:`~pysymex._internal.contracts.types.FunctionContract` records
via weak references. Decorator functions in :mod:`pysymex._internal.contracts.decorators` populate
...
def _get_or_create(func: Callable[..., object]) -> FunctionContract:
...
def _get(func: Callable[..., object]) -> FunctionContract | None:
...
def _source_line(func: Callable[..., object]) -> int | None:
...
def _class_invariants(cls: type[object]) -> list[Contract]:
...
def _attach_contract(func: Callable[..., object], contract: FunctionContract) -> None:
    """Attach contract metadata to a function object."""
    setattr(func, "__contract__", contract)  # noqa: B010


def _attach_invariants(cls: type[object], invariants: list[Contract]) -> None:
    """Attach invariant metadata to a class object."""
    setattr(cls, "__invariants__", invariants)  # noqa: B010
...
class ContractRegistry:
    ...
    source_line = staticmethod(_source_line)
    class_invariants = staticmethod(_class_invariants)
    attach_contract = staticmethod(_attach_contract)
    attach_invariants = staticmethod(_attach_invariants)
