...
:class:`~pysymex._internal.contracts.verifier.ContractVerifier` compiles clauses with
:class:`~pysymex._internal.contracts.compiler.ContractCompiler` and issues bounded incremental
solver queries. Runtime VM hooks in :mod:`pysymex._internal.contracts.runtime` use the
...
from pysymex._internal.contracts.binding.snapshots import is_old_symbol_name
...
from pysymex._internal.core.solver.engine.models import z3_value_to_python
...
class ContractVerifier:
    ...
    Each method returns a :class:`~pysymex._internal.contracts.enums.VerificationResult`
    ...
    def _check_constraints(
        ...
        *,
        ...
        token = SolverContext.active.set(self.solver)
        try:
    ...
    def verify_loop_invariant(
        ...
        if inductive_result.is_sat and inductive_result.model is not None:
            ...
            return VerificationResult.VIOLATED, self.extract_counterexample(model, symbols)
        if inductive_result.is_unsat and base_result.is_unsat:
            return VerificationResult.VERIFIED, None
        return VerificationResult.UNKNOWN, None
