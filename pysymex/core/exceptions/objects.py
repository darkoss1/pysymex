...
from pysymex.core.solver.scoped import check_scoped_constraints
...
class ExceptionHandler:
    """Represent an ``except`` clause and its target program counter."""
    ...
    def catches(self, exc: SymbolicException) -> bool:
        """Return whether this handler may catch ``exc``.

        Limitations:
            Symbolically named exception types are conservatively treated as
            caught because their class relationship is unavailable.
        """
    ...
    def catches_type(self, exc_type: type[BaseException]) -> bool:
        """Return whether this handler catches concrete ``exc_type``."""
        if self.exc_types is None:
            return True
...
class FinallyHandler:
    """Represent the entry and exit program counters of a ``finally`` block."""
...
@dataclass(frozen=True, slots=True)
class TryBlock:
    """Represent handler metadata for one ``try/except/finally`` region."""
