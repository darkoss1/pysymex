...
# Suffixes generated by SymbolicValue's tagged-union encoding.
# Stripping these gives us the base SymbolicValue name, which is
# the correct granularity for the interaction graph.
# IMPORTANT: ordered longest-first so that e.g. "_is_bool" matches
# before "_bool".
...
class ConstraintInteractionGraph:
    ...
    def add_branch(self, pc: int, condition: z3.BoolRef) -> BranchInfo:
        ...
        old_max_degree = max(
            (len(neighbors) for neighbors in self._adjacency.values()), default=0
        )
    ...
    def _compute_degeneracy(self) -> int:
        ...
        # Include isolated branches (no edges)
    ...
    def compute_tree_decomposition(self) -> TreeDecomposition:
        ...
        if not self._branch_info:
            return TreeDecomposition(
                bags={}, tree_edges=[], adhesion={}, width=0
            )

        # Work on a copy of the adjacency
        adj: dict[int, set[int]] = {
            v: set(ns) for v, ns in self._adjacency.items()
        }
        for pc in self._branch_info:
            if pc not in adj:
                adj[pc] = set()
        ...
        # Maps each vertex to the bag where it was eliminated
        vertex_to_bag: dict[int, int] = {}
        bag_id = 0
        max_bag_size = 0

        while remaining:
            # Pick vertex with minimum degree in the remaining subgraph
            v = min(remaining, key=lambda x: len(adj[x] & remaining))
            neighbors_in_remaining = adj[v] & remaining

            # The bag is {v} ∪ neighbors still remaining
            ...
            # Fill-in: connect all neighbors of v to each other
        ...
        for bid in range(len(bags)):
            bag = bags[bid]
            # Find the first later bag that overlaps
...
def _run_self_tests() -> None:
    ...
    graph2.add_branch(1, a < 10)     # shares 'a' with branch 0
    graph2.add_branch(2, b > 0)      # independent
    graph2.add_branch(3, a + b > 5)  # links a and b
    ...
    # Simulate SymbolicValue's multi-Z3-variable encoding
