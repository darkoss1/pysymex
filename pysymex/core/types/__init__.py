...
def symbolic_from_python(value: object) -> SymbolicType:
    ...
    match value:
        ...
        case tuple() as v:  # type: ignore[misc]  # pattern match variable type unknown
            tuple_values: tuple[object, ...] = tuple(v)  # type: ignore[assignment]  # tuple element type unknown at runtime
            elements = tuple(symbolic_from_python(e) for e in tuple_values)
            return SymbolicTuple(elements)
        case list() as v:  # type: ignore[misc]  # pattern match variable type unknown
            list_values: list[object] = list(v)  # type: ignore[assignment]  # list element type unknown at runtime
            if not list_values:
                return SymbolicList.concrete_int_list([])
            if all(isinstance(e, int) for e in list_values):
                int_values: list[int] = [int(e) for e in list_values]  # type: ignore[list-item]  # e is object, but isinstance check ensures int
                return SymbolicList.concrete_int_list(int_values)
