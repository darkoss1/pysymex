...
class SymbolicDictMergeMixin:
    ...
    def update(
        self, other: SymbolicDict | Mapping[object, object] | Mapping[str, object]
        ...
        if isinstance(other, Mapping):
            ...
            if other_concrete is not None:
                ...
                new_keys = cast(
                    "z3.ArrayRef",
                    z3.Lambda(
                        [k], z3.Or(z3.Select(this.known_keys, k), z3.Select(other.known_keys, k))
                    ),
                )
                ...
                return type(this)(
                    _name=f"{getattr(this, '_name')}.update({other.name})",
                    z3_array=new_array,
                    known_keys=new_keys,
                    z3_len=new_len,
                ), constraint
    ...
    def conditional_merge(
        ...
        are_keys_equal = this.known_keys is other.known_keys or safe_z3_eq(
            this.known_keys, other.known_keys
        )
        new_keys = (
            this.known_keys
            if are_keys_equal
