...
Limitations:
    ...
    constraints lives in :mod:`pysymex.core.solver.slices`.
"""
...
def replace_slice_bound(value: SymbolicValue, component: str, bound: StackValue) -> SymbolicValue:
    ...
    if bounds is None:
        raise ValueError("Slice carrier does not retain slice bounds")
...
def possible_zero_step_condition(value: object) -> z3.BoolRef | None:
    ...
    if bounds is None:
        return None
    step = bounds.step
    if step is None or isinstance(step, SymbolicNone):
        return Z3_FALSE
    ...
    if isinstance(step, SymbolicValue):
        if z3.is_true(step.is_none):
            return Z3_FALSE
        if not z3.is_false(step.is_none) and z3.is_true(z3.simplify(step.is_none)):
            return Z3_FALSE
