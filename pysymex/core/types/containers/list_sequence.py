...
class SymbolicListSequenceOpsMixin:
    ...
    def rotate(self, n: int | z3.ArithRef) -> SymbolicList:
        ...
        safe_len = z3.If(this.z3_len == 0, get_int_val(1), this.z3_len)
        shift = n if isinstance(n, z3.ArithRef) else get_int_val(n)

        from pysymex.core.types.scalars.values import py_mod
        ...
        return type(this)(
            _name=f"{getattr(this, '_name')}.rotate({n})",
            z3_array=new_array,
            z3_len=this.z3_len,
    ...
    def extend(self, other: SymbolicList | list[object] | tuple[object, ...]) -> SymbolicList:
        ...
        if isinstance(other, (list, tuple)):
            ...
            if other_concrete is not None:
                ...
                return type(this)(
                    _name=f"{getattr(this, '_name')}.extend({other.name})",
                    z3_array=new_array,
    ...
    def conditional_merge(
        ...
        are_arrays_equal = this.z3_array is other.z3_array or safe_z3_eq(
            this.z3_array, other.z3_array
        )
        new_array = (
            this.z3_array if are_arrays_equal else z3.If(condition, this.z3_array, other.z3_array)
        )

        are_lens_equal = this.z3_len is other.z3_len or safe_z3_eq(this.z3_len, other.z3_len)
        new_len = this.z3_len if are_lens_equal else z3.If(condition, this.z3_len, other.z3_len)

        return type(this)(
            _name=f"If({condition}, {getattr(this, '_name')}, {other.name})",
            z3_array=new_array,
            z3_len=new_len,
            element_type=this.element_type,
        )
