...
CHTD achieves O(N Â· 2^w) structural path exploration via dynamic
...
class ConstraintInteractionGraph:
    ...
    inverted index are updated in amortized O(|vars| Â· max_branches_per_var).
    """
    ...
    def is_stabilized(
        ...
        Returns True when:
        - Enough branches have been seen (â‰¥ min_branches)
    ...
    def compute_tree_decomposition(self) -> TreeDecomposition:
        ...
        for bid in range(len(bags)):
            ...
            if neighbors_in_remaining:
                # The exact parent in the tree decomposition is the bag of the
                # earliest eliminated neighbor in the remaining graph.
                # Since bag IDs are exactly the elimination order indices,
                # we just find the minimum bag ID among the neighbors.
...
def _run_self_tests() -> None:
    ...
    print("=" * 70)
    print("Constraint Interaction Graph â€” Self Tests")
    print("=" * 70)
    ...
    print(f"  PASS â€” treewidth estimate: {graph2.estimated_treewidth}")

    print("\n[TEST 4] Tree decomposition")
    td = graph2.compute_tree_decomposition()
    assert td.width >= 0
    assert len(td.bags) == 4
    print(f"  PASS â€” width={td.width}, bags={len(td.bags)}, edges={len(td.tree_edges)}")
