...
from pysymex.analysis.scan.records import iter_trigger_assignments, normalize_trigger_input
...
class SymbolicRichMixin:
    ...
    def _format_symbolic_rich(
        ...
        if degraded_results:
            summary_grid.add_row(
                "Degraded scans:", f"[bold yellow]{len(degraded_results)}[/bold yellow]"
            )
