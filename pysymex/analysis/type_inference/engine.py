...
class TypeInferenceEngine:
    ...
    def __init__(self) -> None:
        ...
        self._stub_resolver: object = None

    @property
    def stub_resolver(self) -> object:
    ...
    def infer_from_value(self, value: object) -> PyType:
        ...
        if isinstance(value, list):
            if not value:
                return PyType.list_()
            _list_val = cast("list[object]", value)
        ...
        if isinstance(value, dict):
            if not value:
                return PyType.dict_()
            _dict_val = cast("dict[object, object]", value)
            keys: list[object] = list(_dict_val.keys())[:5]
            vals: list[object] = list(_dict_val.values())[:5]
        ...
        if isinstance(value, set):
            if not value:
                return PyType.set_()
            _set_val = cast("set[object]", value)
