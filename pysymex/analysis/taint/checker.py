...
"""Taint analysis core â€” TaintAnalyzer, TaintFlowAnalysis, TaintChecker."""
...
from pysymex.analysis.specialized.flow import (
    BasicBlock,
    CFGBuilder,
    ControlFlowGraph,
    DataFlowAnalysis,
...
class TaintAnalyzer:
    """Bytecode-level taint flow analyser.

    Tracks how tainted data flows through instructions and
    reports when it reaches sensitive sinks.

    Attributes:
        sources: Mapping of source names to ``TaintSource`` definitions.
        sinks: Mapping of sink names to ``TaintSink`` definitions.
        sanitizers: Mapping of sanitizer names to ``Sanitizer`` definitions.
        violations: Accumulated taint violations from the last analysis.
    """
    ...
    def _process_instruction(
        ...
        if opname in {"LOAD_NAME", "LOAD_FAST", "LOAD_GLOBAL", "LOAD_DEREF"}:
            ...
            for _ in range(count * 2):
                if state.stack:
                    elements.append(state.pop())
            result = TaintedValue("dict")
            for elem in elements:
                for label in elem.labels:
                    result.add_label(label.propagate("dict_build"))
            state.push(result)
        elif opname == "POP_TOP":
            if state.stack:
                state.pop()
...
class TaintFlowAnalysis(DataFlowAnalysis[TaintState]):
    ...
    Propagates taint labels through CFG basic blocks, merging state at
    join points.
    """
...
class TaintChecker:
    """High-level facade for taint checking.

    Wraps :class:`TaintAnalyzer` and optionally performs flow-sensitive
    analysis through a CFG.
    """
