...
class ConcurrencyAnalyzer:
    """Comprehensive concurrency analyser using Z3.

    Models thread interleavings and detects data races, deadlocks,
    atomicity violations, and await-graph cycles via SMT solving.
    ...
    def start_thread(
        ...
        if thread is None:
            return None
        thread.state = ThreadState.RUNNING
        if self._thread_op_ids.get(parent_thread):
    ...
    def join_thread(
        ...
        if self._thread_op_ids.get(thread_id):
            ...
            self._thread_op_ids.setdefault(joining_thread, []).append(join_op_id)
    ...
    def detect_data_races(self) -> list[ConcurrencyIssue]:
        ...
        for thread_id, _thread in self._threads.items():
            ...
            for op_id in ops_for_thread:
                op = self._hb_graph.operations.get(op_id)
                if op is None:
                    continue
