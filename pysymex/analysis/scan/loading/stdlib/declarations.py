...
def literal_enum_members(
    node: ast.ClassDef, module_globals: dict[str, object]
) -> dict[str, enum.Enum]:
    ...
    if not literal_members:
        return {}
    concrete_type = base_type(node.name, literal_members)  # pyright: ignore[reportCallIssue]
...
def literal_named_tuple_factory(
    statement: ast.stmt, module_globals: dict[str, object]
    ...
    try:
        created = collections.namedtuple(typename, names)  # pyright: ignore[reportUntypedNamedTuple]
    except (TypeError, ValueError):
        return None
    return statement.targets[0].id, cast("type[object]", created)
...
def _dataclass_field_spec(
    statement: ast.AnnAssign, module_globals: dict[str, object]
