...
"""Bytecode introspection helpers for building exception-flow edges."""
...
from pysymex.analysis.domains.exceptions.types import ExceptionWarning
from pysymex.core.exceptions.builtins import BUILTIN_EXCEPTIONS

_BUILTIN_EXCEPTION_TYPES_BY_NAME: dict[str, type[BaseException]] = {
    exc_type.__name__: exc_type for exc_type in BUILTIN_EXCEPTIONS
}
_CALL_PREFIX_NOOPS = frozenset({"CACHE", "PRECALL", "PUSH_NULL"})
_CONTROL_TRANSFER_OPS = frozenset(
    {
        "JUMP",
        "JUMP_ABSOLUTE",
        "JUMP_BACKWARD",
...
def catches_name(exception_name: str, caught_names: set[str]) -> bool:
...
def infer_with_manager_call_at(
    ...
    while call_index >= 0 and instructions[call_index].opname in {"CACHE", "PRECALL"}:
        call_index -= 1
    if call_index < 0 or instructions[call_index].opname not in {"CALL", "CALL_FUNCTION"}:
        return None
    ...
    type_names: list[str] = []
    cursor = _previous_call_operand_index(instructions, call_index - 1)
    for _ in range(arg_count):
        ...
        cursor = _previous_call_operand_index(instructions, cursor - 1)
    if cursor < 0 or instructions[cursor].opname not in _NAME_LOAD_OPS:
        return None
...
def infer_suppress_call_at(
    instructions: list[dis.Instruction],
    protected_start: int,
    handler_target: int,
) -> tuple[str, tuple[str, ...]] | None:
    """Recognize ``with suppress(ExceptionType, ...)`` around a protected region."""
    call = infer_with_manager_call_at(instructions, protected_start, handler_target)
