...
def symbolic_numeric_zero_condition(
    ...
    *,
    ...
    The value tags may already make one numeric sort unreachable. Simplifying
    here keeps inactive floating-point theory out of integer-only detector
    queries without changing the tagged predicate's semantics.
    """
    int_zero = z3.And(is_int, divisor_zero_condition(int_expr))
    if not include_float:
        return z3.simplify(int_zero)
    return z3.simplify(z3.Or(int_zero, z3.And(is_float, z3.fpIsZero(float_expr))))
...
def tagged_numeric_zero_condition(
    *,
    ...
    Concrete retained payloads and declared affinities avoid constructing inactive
    type-channel branches. Unknown or mixed carriers keep the full tagged predicate.
    """
...
def expression_outside_bounds_condition(
    expr: z3.ExprRef,
    min_val: int,
    max_val: int,
) -> z3.BoolRef:
    """Build the predicate that an integer expression violates inclusive bounds."""
    ...
    return z3.Or(expr < min_val, expr > max_val)


def expression_below_bound_condition(
    expr: z3.ExprRef,
    min_val: int,
) -> z3.BoolRef:
    """Build the predicate that an integer expression is below its inclusive lower bound."""
    if isinstance(expr, z3.BitVecRef):
        expr = z3.BV2Int(expr, is_signed=True)
    elif isinstance(expr, z3.FPRef):
        raise TypeError("Integer underflow check on floating-point is not supported")
    return expr < min_val
