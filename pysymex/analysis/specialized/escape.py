...
class EscapeState(Enum):
    ...
    * ``NO_ESCAPE`` â€“ object never leaves its allocating scope
    * ``ARG_ESCAPE`` â€“ passed as argument (may be captured by callee)
    * ``RETURN_ESCAPE`` â€“ returned from the allocating function
    * ``GLOBAL_ESCAPE`` â€“ stored into a global or attribute (escapes completely)
    """
