...
class CounterexampleExtractor:
    ...
    def extract(self) -> dict[str, object]:
        ...
        counterexample = self._extract_model_declarations(self._z3_model)
        self._extract_missing_constraint_values(self._z3_model, counterexample)
        self._extract_derived_lengths(self._z3_model, counterexample)
        return counterexample

    def _extract_model_declarations(self, model: z3.ModelRef) -> dict[str, object]:
        ...
        for decl in sorted(model.decls(), key=lambda model_decl: model_decl.name()):
            name = decl.name()
            field = self._counterexample_field_name(name)
            if field is None:
                continue
            base_name, field_name = field
            fields_by_base.setdefault(base_name, {})[field_name] = self._z3_value_to_python(
                model[decl]
            )

        counterexample: dict[str, object] = {}
        for base_name in sorted(fields_by_base):
            resolved = self._resolve_counterexample_fields(fields_by_base[base_name])
    ...
    def _counterexample_field_name(name: str) -> tuple[str, str] | None:
        ...
        for suffix, field_name in (
            ...
            if name.endswith(suffix):
                base_name = CounterexampleExtractor._base_counterexample_name(name[: -len(suffix)])
                if base_name is None:
                    return None
                return base_name, field_name

        base_name = CounterexampleExtractor._base_counterexample_name(name)
        if base_name is None:
            return None
    ...
    def _extract_missing_constraint_values(
        self, model: z3.ModelRef, counterexample: dict[str, object]
    ) -> None:
        ...
        for constraint in self.constraints:
            ...
            if var_name in counterexample or var_name.startswith("_"):
                continue
            try:
                counterexample[var_name] = self._z3_value_to_python(model.eval(left))
            except (z3.Z3Exception, TypeError, ValueError):
    ...
    def _extract_derived_lengths(
        ...
        visited: set[int] = set()
        for constraint in self.constraints:
            self._collect_arithmetic_exprs(
                constraint,
                exprs_to_eval,
                visited=visited,
            )
...
__all__ = ["CounterexampleExtractor"]
