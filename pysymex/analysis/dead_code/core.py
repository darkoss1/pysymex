...
class UnreachableCodeDetector:
    ...
    def detect(
        ...
        if unreachable and unreachable_start:
            if is_genexpr or is_generator or is_async or is_async_gen:
                pass

            elif self._is_only_implicit_return(
    ...
    def _region_has_user_code(
        ...
        if start_idx is None or terminator_line is None:
            return False
        ...
        for j in range(start_idx, end_idx):
            ...
            if line_val is not None and line_val > terminator_line:
                return True
...
class UnusedVariableDetector:
    ...
    @staticmethod
    def _collect_nested_uses(code: Any) -> set[str]:
                ...
                for instr in _cached_get_instructions(const):
                    if instr.opname in {
                        ...
                        if isinstance(instr.argval, tuple):
                            for n in cast(tuple[Any, ...], instr.argval):
                                uses.add(str(n))
...
class DeadStoreDetector:
    ...
    def detect(
        ...
        for instr in instructions:
            ...
            if opname in {"STORE_FAST", "STORE_NAME"}:
            ...
            elif opname in {"LOAD_FAST", "LOAD_NAME", "LOAD_FAST_AND_CLEAR"}:
                name = str(arg)
                ...
                if isinstance(arg, tuple) and len(arg) >= 2:
                    ...
                    if lname in last_store:
                        del last_store[lname]
...
class UnusedParameterDetector:
    ...
    @staticmethod
    def _is_stub_body(code: Any) -> bool:
