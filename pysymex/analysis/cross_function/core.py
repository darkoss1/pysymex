...
class FunctionSummaryCache:
    ...
    def get(
        self, func_name: str, args: list[Any], path_constraints: list[z3.BoolRef]
    ) -> Any | None:
    ...
    def put(
        self, func_name: str, args: list[Any], path_constraints: list[z3.BoolRef], summary: Any
    ) -> None:
    ...
    def _compute_key(
        ...
        for i, arg in enumerate(args):
            ...
            if isinstance(arg, SymbolicValue):
                ...
                sym_args.append(f"{arg_type}:{str(arg)}")
        ...
        if not path_constraints:
            ...
            for c in relevant_slice:
                if canonical_map:
                    canonical_constraints.append(cast(z3.BoolRef, z3.substitute(c, *canonical_map)))
...
def _infer_return_type(code: Any) -> PyType | None:
