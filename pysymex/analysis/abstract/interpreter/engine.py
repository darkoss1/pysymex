...
from pysymex.analysis.abstract.interpreter.values import Congruence, Interval, SignValue
...
class AbstractInterpreter:
    ...
    **Convergence Mechanics:**
    ...
    def _refine_successor_state(
        ...
        """Refine a successor using a simple numeric comparison terminator."""
    ...
    def _simple_numeric_branch_comparison(
        self,
        block: BasicBlock,
    ) -> tuple[str, str, int] | None:
        """Return `var op const` for the common compare/jump bytecode shape."""
    ...
    def _refine_variable_comparison(
        ...
        *,
        ...
        result.set(
            variable,
            result.get(variable).meet(
                NumericProduct(refined_range, SignValue.top(), Congruence.top())
            ),
        )
        return result

    @staticmethod
    def _comparison_range(op: str, constant: int, *, assume_true: bool) -> Interval | None:
        if not assume_true:
            inverted = {
                "<": ">=",
                "<=": ">",
            ...
            if inverted is None:
                return None
            return AbstractInterpreter._comparison_range(inverted, constant, assume_true=True)
        if op == "<":
            return Interval.range(None, constant - 1)
        if op == "<=":
            return Interval.range(None, constant)
        if op == ">":
            return Interval.range(constant + 1, None)
        if op == ">=":
            return Interval.range(constant, None)
        if op == "==":
    ...
    def _comparison_excludes_singleton(
        current: NumericProduct,
        op: str,
        constant: int,
        *,
        assume_true: bool,
    ) -> bool:
        if not current.interval.is_const() or current.interval.get_const() != constant:
            return False
