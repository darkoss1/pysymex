...
class Interval(AbstractValue["Interval"]):
    ...
    def __mul__(self, other: Interval) -> Interval:
        ...
        if any(e is None for e in endpoints):
            ...
            if corners:
                _finite_lo = min(corners)

                _finite_hi = max(corners)

                result_lo = _finite_lo if result_lo is not None else None

                result_hi = _finite_hi if result_hi is not None else None
