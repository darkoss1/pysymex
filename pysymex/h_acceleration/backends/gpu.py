...
if TYPE_CHECKING:
    ...
    CupyStream: TypeAlias = cupy.cuda.Stream
...
MAX_INSTRUCTIONS: int = 4096  # Informational; actual validation in bytecode.py (MAX=8192)
THREAD_COARSENING: int = 8
THREADS_PER_BLOCK: int = 256  # Default baseline; actual value computed adaptively in _get_optimal_launch_config
...
def is_available() -> bool:
    ...
    try:
        ...
        # Accessing compute_capability verifies the device is accessible
...
def _get_device_info() -> dict[str, str | int]:
    ...
    with _device_lock:
        ...
        try:
            if cp is None: return {"error": "CuPy not installed"}
            dev = cp.cuda.Device()
...
def _instruction_hash(constraint: CompiledConstraint) -> str:
    ...
    return hashlib.sha256(constraint.instructions.tobytes()).hexdigest()[:16]
...
def _gen_cuda_source(
    ...
    lines.append(f'    for (unsigned long long base = tid * {THREAD_COARSENING}; base < num_chunks; base += stride * {THREAD_COARSENING}) {{')
    ...
    for i in range(6, num_vars):
        if i in used_vars:
            bit = i - 6
            for c in range(THREAD_COARSENING):
                lines.append(f'        unsigned long long r{i + 1}_{c} = 0ULL - ((chunk{c} >> {bit}ULL) & 1ULL);')
    ...
    for instr in instrs:
        ...
        dst, s1, s2, imm = int(instr['dst']), int(instr['src1']), int(instr['src2']), int(instr['immediate'])

        for c in range(THREAD_COARSENING):
            if op == Opcode.AND: lines.append(f'        r{dst}_{c} = r{s1}_{c} & r{s2}_{c};')
            elif op == Opcode.OR: lines.append(f'        r{dst}_{c} = r{s1}_{c} | r{s2}_{c};')
            elif op == Opcode.NOT: lines.append(f'        r{dst}_{c} = ~r{s1}_{c};')
            elif op == Opcode.XOR: lines.append(f'        r{dst}_{c} = r{s1}_{c} ^ r{s2}_{c};')
            elif op == Opcode.LOAD_VAR: lines.append(f'        r{dst}_{c} = r{imm + 1}_{c};')
            elif op == Opcode.LOAD_TRUE: lines.append(f'        r{dst}_{c} = 0xFFFFFFFFFFFFFFFFULL;')
            elif op == Opcode.LOAD_FALSE: lines.append(f'        r{dst}_{c} = 0ULL;')
            elif op == Opcode.COPY: lines.append(f'        r{dst}_{c} = r{s1}_{c};')
            elif op == Opcode.IMPLIES: lines.append(f'        r{dst}_{c} = (~r{s1}_{c}) | r{s2}_{c};')
            elif op == Opcode.IFF or op == Opcode.EQ: lines.append(f'        r{dst}_{c} = ~(r{s1}_{c} ^ r{s2}_{c});')
            elif op == Opcode.NE: lines.append(f'        r{dst}_{c} = r{s1}_{c} ^ r{s2}_{c};')
            elif op == Opcode.ITE: lines.append(f'        r{dst}_{c} = (r{s1}_{c} & r{s2}_{c}) | ((~r{s1}_{c}) & r{imm}_{c});')
            elif op == Opcode.NAND: lines.append(f'        r{dst}_{c} = ~(r{s1}_{c} & r{s2}_{c});')
            elif op == Opcode.NOR: lines.append(f'        r{dst}_{c} = ~(r{s1}_{c} | r{s2}_{c});')
    ...
    for c in range(THREAD_COARSENING):
        ...
        lines.append(f'                const unsigned long long remaining = num_states - base_tid{c};')
    ...
    if mode == 'count':
        ...
        for c in range(THREAD_COARSENING):
            ...
            lines.append('                                    adhesion_idx |= (1U << adhesion_mapping[i]);')
            lines.append('                                } ')
            lines.append('                            } ')
            lines.append('                        } ')
...
def _get_kernel(constraint: CompiledConstraint, mode: str) -> CupyKernel:
    if cp is None: raise BackendError("CuPy not installed")
    ...
    try:
        module = cp.RawModule(code=src, options=('-std=c++11', '--use_fast_math'), name_expressions=[kernel_name])
...
def evaluate_bag(constraint: CompiledConstraint) -> npt.NDArray[np.uint8]:
    ...
    with _bitmap_lock:
        if cache_key in _bitmap_cache:
            ...
            return cp.asnumpy(d_bitmap).view(np.uint8)[:((num_states + 7) // 8)]
...
def evaluate_bag_async(constraint: CompiledConstraint, stream: CupyStream | None = None) -> tuple[CupyArrayU64, CupyStream]:
    if not is_available() or cp is None:
        raise BackendError("CuPy GPU backend not available")
    from pysymex.h_acceleration.bytecode_optimizer import optimize
    constraint, _ = optimize(constraint)
    ...
    kernel = _get_kernel(constraint, mode='eval')
    blocks, threads = _get_optimal_launch_config(constraint, num_chunks)
    with stream:  # pyright: ignore[reportOptionalContextManager]
        kernel((blocks,), (threads,), (np.uint64(num_chunks), np.uint64(num_states), d_output))
    return d_output, stream

def evaluate_bag_projected(constraint: CompiledConstraint, adhesion_vars: list[str], bag_vars: list[str]) -> npt.NDArray[np.uint8]:
    if not is_available() or cp is None:
        raise BackendError("CuPy GPU backend not available")
    from pysymex.h_acceleration.bytecode_optimizer import optimize
    constraint, _ = optimize(constraint)
    ...
    if not adhesion_indices: return projected
    cache_key = f"{_bitmap_cache_key(constraint)}_proj_{hash(tuple(adhesion_indices))}"
    with _bitmap_lock:
        if cache_key in _bitmap_cache:
            d_bitmap, _ = _bitmap_cache[cache_key]
            _bitmap_cache.move_to_end(cache_key)
            return cast("npt.NDArray[np.uint8]", cp.asnumpy(d_bitmap).view(np.uint8)[:((num_adhesion_states + 7) // 8)])
    w = constraint.num_variables
    num_states = 1 << w
    num_chunks = (num_states + 63) // 64
    ...
    blocks, threads = _get_optimal_launch_config(constraint, num_chunks)
    kernel((blocks,), (threads,), (np.uint64(num_chunks), np.uint64(num_states), d_output, adhesion_mask, d_mapping))
    cp.cuda.Device().synchronize()
    with _bitmap_lock:
        _bitmap_cache[cache_key] = (d_output, w_adhesion)
        while len(_bitmap_cache) > BITMAP_CACHE_SIZE:
            _, (old_buf, _) = _bitmap_cache.popitem(last=False)
            if hasattr(old_buf, 'dtype') and old_buf.dtype == np.uint64:
                _buffer_pool.release(old_buf)
    return cp.asnumpy(d_output).view(np.uint8)[:((num_adhesion_states + 7) // 8)]
...
def clear_caches() -> None:
    ...
    with _kernel_lock: _kernel_cache.clear()
    with _bitmap_lock:
        for _, (buf, _) in _bitmap_cache.items(): _buffer_pool.release(buf)
        _bitmap_cache.clear()
