...
Used by :mod:`pysymex.contracts.quantifiers.extraction` and
:mod:`pysymex.contracts.quantifiers.factories` to build
:class:`~pysymex.contracts.quantifiers.types.Quantifier` values. Does not run solver
checks.
"""
...
from pysymex.contracts.quantifiers.types import BoundSpec, Quantifier, QuantifierKind, QuantifierVar
...
class QuantifierParser:
    ...
    def _parse_bounds(self, range_str: str, var_name: str) -> BoundSpec:
        ...
        if range_match:
            ...
            if matched_var != var_name:
                raise ValueError(
                    f"Quantifier bound variable {matched_var!r} does not match {var_name!r}"
                )
            return BoundSpec(
        ...
        if in_match:
            ...
            if collection_expr is None:
                raise ValueError(f"Unbound quantifier collection: {collection}")
            return BoundSpec(in_collection=collection_expr)
        raise ValueError(f"Unsupported quantifier bounds: {range_str!r}")
    ...
    def _parse_expr(self, expr_str: str) -> z3.ExprRef:
        ...
        try:
            return get_int_val(int(expr_str))
        except ValueError:
        ...
        if expr_str.startswith("len(") and expr_str.endswith(")"):
            ...
            if length_expr is not None:
                return length_expr
        raise ValueError(f"Unbound quantifier bound expression: {expr_str}")
    ...
    def _parse_body(self, body_str: str, local_vars: dict[str, z3.ExprRef | None]) -> z3.BoolRef:
        ...
        if not z3.is_bool(res):
            raise ValueError(f"Quantifier body is not boolean: {body_str!r}")
        return res


__all__ = ["QuantifierParser", "QuantifierSyntax", "parse_quantifier_syntax"]
