...
from pysymex.core.solver.constraints.hashing import get_bool_val, get_int_val, get_string_val
...
def length_snapshot_expression(value: object, memory: object | None = None) -> z3.ArithRef | None:
    ...
    if isinstance(resolved, (str, bytes)):
        return get_int_val(len(resolved))
    if isinstance(resolved, tuple):
        return get_int_val(len(cast("tuple[object, ...]", resolved)))
    if isinstance(resolved, list):
        return get_int_val(len(cast("list[object]", resolved)))
    if isinstance(resolved, dict):
        return get_int_val(len(cast("dict[object, object]", resolved)))
    if isinstance(resolved, (set, frozenset)):
        return get_int_val(len(cast("set[object] | frozenset[object]", resolved)))
...
def collect_current_derived_symbols(
    local_vars: Mapping[str, "StackValue"],
...
def runtime_frame_parts(
    frame: object,
) -> tuple[Callable[..., object], Mapping[str, z3.ExprRef], int] | None:
    """Return the function and old-symbols carried by a contract frame."""
    if isinstance(frame, RuntimeContractFrame):
        return frame.function, frame.old_symbols, frame.effect_start_index
    if callable(frame):
        return frame, _empty_old_symbols(), 0
    return None
...
__all__ = [
    ...
    "collect_scalar_old_symbols",
    "is_old_symbol_name",
    "length_snapshot_expression",
    "old_symbol_name",
