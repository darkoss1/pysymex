...
def _format_issue_html(issue: IssueReport) -> str:
    """Render a single *issue* as an HTML card fragment.

    Args:
        issue: The issue to render.

    Returns:
        HTML string for embedding inside the issues section.
    """
    severity_class = (
...
def _format_issues_section(issues: list[IssueReport]) -> str:
    ...
    Returns a *"No Issues Found"* placeholder when *issues* is empty.
...
def _format_resources_html(resources: ResourceSnapshot | None) -> str:
    """Render resource-usage metrics as HTML stat cards.

    Args:
        resources: Snapshot of engine resource usage, or ``None``.
...
def create_report_from_result(
    ...
    if is_object_list(raw_issues):
        for issue in raw_issues:
            ...
            line_number = _optional_int(getattr(issue_obj, "line_number", None))
            triggering_input = _optional_input_dict(getattr(issue_obj, "triggering_input", None))
