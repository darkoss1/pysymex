...
"""Helper serializers and protocols for execution tracing."""
...
from pysymex.tracing.schemas import ConstraintEntry
...
def to_config_scalar(value: object) -> str | int | float | bool | None:
...
def normalise_config_snapshot(
    snapshot: dict[str, object],
) -> dict[str, str | int | float | bool | None]:
    """Convert a raw config snapshot to ``SystemContextEvent`` scalar schema."""
    return {str(key): to_config_scalar(value) for key, value in snapshot.items()}


def elapsed_ms_since(start_ns: int | None) -> float | None:
...
def serialize_constraint_entries(
