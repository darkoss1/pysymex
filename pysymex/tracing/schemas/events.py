...
class SystemContextEvent(BaseModel):
    ...
    initial_symbolic_args: dict[str, str] = Field(default_factory=new_string_dict)
    tracer_config: dict[str, ConfigScalar] = Field(default_factory=new_config_scalar_dict)
...
class StepDeltaEvent(BaseModel):
    ...
    mem_diff: dict[str, str] = Field(default_factory=new_string_dict)
...
class KeyframeEvent(BaseModel):
    ...
    pc: int = 0
    depth: int = 0
    stack: list[str] = Field(default_factory=new_string_list)
    local_vars: dict[str, str] = Field(default_factory=new_string_dict)
    global_vars: dict[str, str] = Field(default_factory=new_string_dict)
    path_constraints: list[ConstraintEntry] = Field(default_factory=new_constraint_list)
...
class DetectorQueryTraceEvent(BaseModel):
    ...
    constraint_excerpt: list[ConstraintEntry] = Field(default_factory=new_constraint_list)
...
class PathFeasibilityTraceEvent(BaseModel):
    ...
    query_constraint_excerpt: list[ConstraintEntry] = Field(default_factory=new_constraint_list)
...
class IssueEvent(BaseModel):
    ...
    constraints_at_issue: list[ConstraintEntry] = Field(default_factory=new_constraint_list)
