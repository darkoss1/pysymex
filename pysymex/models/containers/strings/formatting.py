    ...
    def apply(
        ...
        if original is not None:
            ...
            if isinstance(width, int):
                constraints.append(
                    z3.Or(result.z3_len == get_int_val(width), result.z3_len == original.z3_len)
                )
...
def _exact_padding_result(
    ...
    if literal is not None and width is None and len(args) > 1 and _definitely_not_int(args[1]):
        return method_type_error_result(qualname, state)
    if literal is None or width is None:
        return None

    fillchar = " "
    if len(args) == 3:
        concrete_fillchar = concrete_string_literal(args[2])
...
def _definitely_not_string(value: object) -> bool:
    if isinstance(value, SymbolicValue):
        value = value.value
    return not isinstance(value, (str, SymbolicString)) and value is not None
