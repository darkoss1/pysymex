    ...
    *args: object,
    ...
    if len(args) == 1:
        ...
        if isinstance(start, int) and isinstance(stop, int) and isinstance(step, int):
            ...
            result.z3_len = get_int_val(expected_len)
...
def _model_default_accumulate_items(
    ...
    if func is not None and not isinstance(func, SymbolicNone):
        return None
    items = iterable.concrete_items
    if items is None:
        return None
...
class AccumulateModel(FunctionModel):
    ...
    def apply(
        ...
        iterable = get_symbolic_list(args[0], state) if args else None
        if iterable is None:
...
class ChainModel(FunctionModel):
    ...
    def apply(
        ...
        for arg in args:
            iterable = get_symbolic_list(arg, state)
            if iterable is None:
                result, constraint = SymbolicList.symbolic(f"chain_{state.pc}")
                return ModelResult(value=result, constraints=[constraint])
