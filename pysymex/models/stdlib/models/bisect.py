...
def _bisect_position(values: list[int], needle: SymbolicValue, *, right: bool) -> z3.ArithRef:
    position: z3.ArithRef = get_int_val(len(values))
    for index, item in reversed(list(enumerate(values))):
        comparison = needle.z3_int < item if right else needle.z3_int <= item
        position = z3.If(comparison, get_int_val(index), position)
    return position


def _bisect_index_result(
    prefix: str, args: list[StackValue], state: VMState, *, right: bool
) -> ModelResult:
    result, constraint = SymbolicValue.symbolic(f"{prefix}_{state.pc}")
