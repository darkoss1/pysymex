    ...
    for instruction in instructions[:scan_end]:
        ...
        if opname == "CALL":
            ...
            if nested is not None:
                ...
                return PotentialRaise(nested.type_name, nested.message, z3.simplify(condition))
            return None
...
def _build_potential_raise(
    ...
    if argc == 0 or len(stack) < argc:
        return None
    exc = stack[-argc]
    condition = z3.simplify(z3.And(*guards)) if guards else Z3_TRUE
...
def _function_globals(function: object, state: VMState) -> dict[str, object]:
    ...
    merged: dict[str, object] = {
        name: exception_type for name, exception_type in _builtin_exception_types().items()
    }
...
def _compare_symbolic_to_const(
    ...
    if isinstance(constant, int):
        numeric_expr = z3.If(
            value.is_bool,
            z3.If(value.z3_bool, get_int_val(1), get_int_val(0)),
            value.z3_int,
        )
        return z3.And(
            z3.Or(value.is_int, value.is_bool),
            _compare_arith(numeric_expr, op_name, get_int_val(constant)),
        )
    if isinstance(constant, str) and op_name in {"==", "!="}:
        condition = z3.And(value.is_str, value.z3_str == get_string_val(constant))
...
def _compare_arith(left: z3.ArithRef, op_name: str, right: z3.ArithRef) -> z3.BoolRef:
    ...
    if op_name == "<=":
        return left <= right
    raise TypeError(f"unsupported comparison op: {op_name}")
...
def _compare_concrete(left: object, op_name: str, right: object) -> bool:
    ...
    raise TypeError(f"unsupported comparison operands: {left_type}, {right_type}")


def _compare_numeric(left: int | float | bool, op_name: str, right: int | float | bool) -> bool:
    if op_name == ">":
        return left > right
    if op_name == ">=":
        return left >= right
