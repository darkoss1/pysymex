...
from pysymex.models.typed_results import symbolic_bool_result

from ...base import ModelResult
from .exceptions import modeled_generator_call_exceptions
from .sources import generator_code as _generator_code
from .sources import generator_items as _generator_items
...
def modeled_generator_truth(
    *,
    ...
    code = _generator_code(generator)
    if code is None:
        return None
    ...
    items = _generator_items(generator, state)
    if items is None:
        return None
...
def _compare_arith(
    ...
    if op_name == "<=":
        return left <= right
    raise ValueError(f"unsupported comparison op: {op_name}")
...
def _compare_item_to_constant(item: object, op_name: str, constant: object) -> z3.BoolRef | None:
    ...
    if isinstance(item, SymbolicValue):
        if constant is None:
            ...
            if op_name not in {"==", "!="}:
                return None
            condition = z3.And(item.is_str, item.z3_str == get_string_val(constant))
        else:
            return None
