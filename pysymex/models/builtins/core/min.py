...
from .helpers import (
    ...
    safe_min_concrete as _safe_min_concrete,
    ...
    def apply(
        ...
        if not args:
            ...
            return ModelResult(
                ...
                side_effects=_type_error_side_effect(
                    "builtins.min", "min expected at least 1 argument, got 0"
                ),
            )
        if set(kwargs) - {"key", "default"} or (len(args) > 1 and "default" in kwargs):
            ...
            return ModelResult(
                ...
                constraints=[constraint],
                side_effects=_type_error_side_effect(
                    "builtins.min", "min() received invalid keyword arguments"
                ),
            )
        ...
        if sequence is not None:
            ...
            symbolic_min = _symbolic_extreme_result(seq, f"min_{state.pc}", "min")
            if symbolic_min is not None:
                return _model_result_with_side_effects(symbolic_min, iterator_side_effects)
            if all(
