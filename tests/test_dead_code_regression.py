...
"""
    ...
    def test_real_dead_code_still_detected(self):
        ...
        # Use module-level code where CPython tends to preserve more bytecode
        source = """\
def compute():
    return 42
    x = 10
...
"""
        ...
        # which means there's nothing for the bytecode detector to find.
        # This is not a detector bug — the dead code was removed by the
        # compiler.  The test verifies the detector doesn't crash and
        # doesn't produce false positives on whatever bytecode remains.
        import dis
...
# ==============================================================================
# UNREACHABLE_CODE: generators, async, genexprs
# ==============================================================================
...
"""
        ...
        detector = UnreachableCodeDetector()
        # Check function-level code
        issues = detector.detect(code, "<test>")
        # Also check nested genexpr code objects
...
# ==============================================================================
# UNREACHABLE_CODE: PUSH_EXC_INFO as implicit jump target
# ==============================================================================
...
"""
        ...
        detector = UnusedVariableDetector()
        issues = detector.detect(code, "<test>")
        # The loop variable 'x' and the 'ids' result should not be flagged
        unused = _find_issues(issues, DeadCodeKind.UNUSED_VARIABLE)
        var_names = {i.name for i in unused}
...
"""
    ...
    def test_file_read_not_taint_source_when_dotted(self):
        ...
        analyzer = TaintAnalyzer()
        # Look up the source for a dotted name with ambiguous base
        source = analyzer._find_source("response.read")
        # Should be None because 'read' is in _AMBIGUOUS_SHORT_NAMES
        # and the full qualified name 'response.read' is not in the source list
    ...
    def test_qualified_source_still_matches(self):
        ...
        assert "NETWORK" in source.kind.name  # type: ignore[reportOperatorIssue]
    ...
    def test_bare_input_still_matches(self):
        ...
        assert "USER_INPUT" in source.kind.name  # type: ignore[reportOperatorIssue]
    ...
    def test_dotted_ambiguous_sink_suppressed(self):
        ...
        # 'cursor.execute' is now registered as a qualified sink name
        # pointing to the 'execute' SQL sink definition.
...
# ==============================================================================
# DeadCodeAnalyzer integration: full module analysis
# ==============================================================================
...
"""
        ...
        code = _compile_and_get_code(source)
        results = analyzer.analyze_module(code, source, "<test>")
        # Filter to only items from our detectors (not unused imports etc.)
        unreachable = _find_issues(results, DeadCodeKind.UNREACHABLE_CODE)
        assert len(unreachable) == 0
...
"""
        ...
        code = _compile_and_get_code(source)
        results = analyzer.analyze_module(code, source, "<test>")
        # Unused parameter 'c' should always be detectable regardless of
...
# ==============================================================================
# RedundantConditionDetector: TO_BOOL invalidation
# ==============================================================================
...
"""
        ...
        detector = RedundantConditionDetector()
        issues = detector.detect(code, "<test>")
        # Note: CPython may optimize this away, so be lenient
        # The test verifies the detector doesn't crash on this pattern


# ==============================================================================
# _region_has_user_code: unit tests for the heuristic
# ==============================================================================
