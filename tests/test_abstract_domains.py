...
class TestInterval:
    ...
    def test_sub(self):
        ...
        assert result.lo == 5  # 10 - 5
        assert result.hi == 19  # 20 - 1
...
class TestProductDomain:
    ...
    def test_to_z3_constraint(self):
        ...
        # x = 4: in [0,10], positive, even
        solver = z3.Solver()
        solver.add(constraint)
        solver.add(x == 4)
        assert solver.check() == z3.sat

        # x = 5: positive, odd (not even)
        solver2 = z3.Solver()
        solver2.add(constraint)
        solver2.add(x == 5)
        assert solver2.check() == z3.unsat
...
class TestAbstractState:
    ...
    def test_widen_states(self):
        ...
        # Extended in both directions -> unbounded
        assert x_value.interval.is_top()
...
class TestAbstractInterpreter:
    ...
    def test_analyze_loop(self):
        ...
        # Should converge to unbounded due to widening
        x_value = final.get("x")
        # After widening, the upper bound goes to infinity
...
class TestUseCases:
    ...
    def test_array_bounds(self):
        ...
        # index in [0, len-1], len = 10
        index = Interval.range(0, 9)
        length = Interval.from_concrete(10)

        # index < length should be provable
        assert index.hi < length.lo  # type: ignore[reportOperatorIssue]

    def test_division_by_zero(self):
        """Division by zero with sign domain."""
        # x / y where y is positive
    ...
    def test_null_dereference(self):
        ...
        # After: if x is not None: use(x)
        # x should be non-null in the body

        x = Null.top()  # Initially unknown

        # Refine based on condition x is not None
