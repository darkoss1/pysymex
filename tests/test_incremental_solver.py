...
class TestCaching:
    ...
    def test_cache_survives_pop_back_to_parent_scope(self):
        solver = IncrementalSolver()
        x = z3.Int("x")

        solver.add(x > 0)
        assert solver.is_sat([x < 10]) is True
        initial_queries = solver._query_count

        solver.push()
        solver.add(x < 0)
        assert solver.is_sat([x < 10]) is False
        assert solver._query_count > initial_queries

        solver.pop()
        assert solver.is_sat([x < 10]) is True
        assert solver._query_count == initial_queries + 1
        assert solver._cache_hits >= 1

    def test_cache_distinguishes_ambient_constraints(self):
        solver = IncrementalSolver()
        x = z3.Int("x")

        solver.add(x > 0)
        assert solver.is_sat([x < 10]) is True
        queries_after_first = solver._query_count
...
class TestWarmStart:
    ...
    def test_warm_start_stores_models(self):
        ...
        x = z3.Int("x")
        solver.is_sat([x > 0])
        assert len(solver._last_models) >= 0  # May or may not store depending on SAT
