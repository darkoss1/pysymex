...
class TestTypeEncoder:
    ...
    def test_encode_caching(self, encoder):
        ...
        # Should return same Z3 constant
        assert z1 is z2
    ...
    def test_get_axioms(self, encoder):
        ...
        assert len(axioms) > 0
        # All axioms should be Z3 expressions
        for ax in axioms:
            assert isinstance(ax, z3.ExprRef)


# =============================================================================
# TypeConstraintChecker Tests
# =============================================================================
...
class TestTypeConstraintChecker:
    ...
    def test_check_union_access_honors_runtime_narrowing(self, checker):
        """Runtime type constraints should suppress impossible union members."""
        union_t = SymbolicType.union_of(SymbolicType.str_type(), SymbolicType.int_type())
        runtime_t = z3.Const("runtime_t", checker.encoder.TypeSort)

        issue = checker.check_union_access(
            union_t,
            "upper",
            path_constraints=[runtime_t == checker.encoder.encode(SymbolicType.str_type())],
            runtime_type=runtime_t,
        )

        assert issue is None

    def test_check_none_safety_honors_runtime_narrowing(self, checker):
        """Non-None runtime constraints should suppress optional warnings."""
        optional_t = SymbolicType.optional_of(SymbolicType.str_type())
        runtime_t = z3.Const("runtime_t", checker.encoder.TypeSort)

        issue = checker.check_none_safety(
            optional_t,
            operation="call upper",
            path_constraints=[runtime_t == checker.encoder.encode(SymbolicType.str_type())],
            runtime_type=runtime_t,
        )
