"""Precision benchmark for pysymex v0.5.0.
...
Target: >20% precision (ROADMAP v0.5.0)
...
class TestPrecisionBenchmark:
    """Precision benchmark: >20% target (ROADMAP v0.5.0)."""
    ...
    def test_clean_code_low_fp(self):
        ...
        assert total_fp < len(KNOWN_CLEAN), (
            f"Too many FPs in clean code: {total_fp} " f"(should be < {len(KNOWN_CLEAN)})"
        )
