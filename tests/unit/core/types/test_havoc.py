...
def test_union_taint() -> None:
    """Scenario: merge taint labels across values; expected full label union."""
    left, _ = pysymex.core.types.havoc.HavocValue.havoc("h3", taint_labels={"a"})
    right, _ = pysymex.core.types.havoc.HavocValue.havoc("h4", taint_labels={"b"})
    assert pysymex.core.types.havoc.union_taint([left, right]) == frozenset({"a", "b"})
