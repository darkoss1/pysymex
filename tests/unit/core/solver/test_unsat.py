...
import pysymex._internal.core.solver.unsat


class TestUnsatCoreResult:
    """Test suite for pysymex._internal.core.solver.unsat.UnsatCoreResult."""

    def test_reduction_ratio(self) -> None:
        """Scenario: one core constraint out of two total; expected 0.5 reduction ratio."""
        result = pysymex._internal.core.solver.unsat.UnsatCoreResult([z3.BoolVal(False)], [0], 2)
...
def test_extract_unsat_core() -> None:
    ...
    x = z3.Int("x")
    core = pysymex._internal.core.solver.unsat.extract_unsat_core([x > 0, x <= 0])
...
def test_extract_unsat_core_check_failure_returns_none(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert pysymex._internal.core.solver.unsat.extract_unsat_core([x > 0, x <= 0]) is None
...
def test_core_indicator_names_are_deterministic(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    first = pysymex._internal.core.solver.unsat.extract_unsat_core([x > 0, x <= 0])

    monkeypatch.setattr(z3, "Bool", capture_second)
    second = pysymex._internal.core.solver.unsat.extract_unsat_core([x > 0, x <= 0])
...
def test_prune_with_core() -> None:
    ...
    core = pysymex._internal.core.solver.unsat.UnsatCoreResult([constraints[1]], [1], 3)
    assert pysymex._internal.core.solver.unsat.prune_with_core(constraints, core) == [
        constraints[1]
    ]
