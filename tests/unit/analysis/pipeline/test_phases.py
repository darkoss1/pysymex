...
from pysymex.analysis.pipeline.phases import (
    ...
    apply_exception_suppression,
    is_used_in_annotations,
...
def make_dummy_ctx() -> AnalysisContext:
    ...
    return AnalysisContext(file_path="f.py", source="def f(): pass", code=code)
...
class TestBugDetectionPhase:
    ...
    def test_analyze(self, mock_filter_issue) -> None:
        ...
        p.analyzer.analyze_function = Mock(
            return_value=[Issue(IssueKind.DIVISION_BY_ZERO, Severity.HIGH, "f.py", 10, "msg")]
        )
        mock_filter = Mock()
...
def test_apply_common_suppression() -> None:
    ...
    ctx = make_dummy_ctx()
    issue = ScanIssue(
        IssueCategory.BUG, "UNUSED_VARIABLE", "warning", "f.py", 10, "Variable 'x' is unused", 0.9
    )
    apply_common_suppression(issue, ctx)
    issue2 = ScanIssue(
        IssueCategory.BUG, "UNUSED_VARIABLE", "warning", "f.py", 10, "Variable '_y' is unused", 0.9
    )
