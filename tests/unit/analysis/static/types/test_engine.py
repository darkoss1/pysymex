...
class TestTypeInferenceEngine:
    ...
    def test_stub_resolver(self) -> None:
        ...
        resolver = engine.stub_resolver
        assert resolver is None or isinstance(resolver, StubBasedTypeResolver)
        assert engine.stub_resolver is resolver
        assert vars(engine)["_stub_resolver"] is not False
    ...
    def test_infer_from_annotation(self) -> None:
        ...
        assert engine.infer_from_annotation(int | str).kind == TypeKind.UNION
        optional = engine.infer_from_annotation(int | None)
        assert optional.kind == TypeKind.UNION
        assert any(member.kind == TypeKind.NONE for member in optional.union_members)
        assert engine.infer_from_annotation("int").kind == TypeKind.INT
    ...
    def test_infer_function_signature_for_uninspectable_builtin(self) -> None:
        """Uninspectable builtins degrade to an explicit unknown signature."""
        engine = TypeInferenceEngine()

        params, ret = engine.infer_function_signature(vars)

        assert params == []
        assert ret.kind == TypeKind.ANY
    ...
    def test_infer_bool_binary_ops_as_int_subclass(self) -> None:
        """bool participates in arithmetic and repetition like CPython int subclassing."""
        engine = TypeInferenceEngine()

        assert (
            engine.infer_binary_op_result("+", PyType.bool_(), PyType.int_()).kind == TypeKind.INT
        )
        assert (
            engine.infer_binary_op_result("*", PyType.bool_(), PyType.str_()).kind == TypeKind.STR
        )
        assert (
            engine.infer_binary_op_result("*", PyType.list_(PyType.int_()), PyType.bool_()).kind
            == TypeKind.LIST
        )
    ...
    def test_infer_unary_op_result(self) -> None:
        ...
        assert engine.infer_unary_op_result("-", PyType.bool_()).kind == TypeKind.INT
        assert engine.infer_unary_op_result("~", PyType.bool_()).kind == TypeKind.INT
