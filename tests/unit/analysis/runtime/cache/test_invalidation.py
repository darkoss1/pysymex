...
class TestInvalidationStrategy:
    """Test suite for pysymex.analysis.runtime.cache.invalidation.InvalidationStrategy."""
...
class TestInvalidationRule:
    """Test suite for pysymex.analysis.runtime.cache.invalidation.InvalidationRule."""
...
class TestSmartInvalidator:
    """Test suite for pysymex.analysis.runtime.cache.invalidation.SmartInvalidator."""
    ...
    def test_is_stale(self, tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
        """Test is_stale behavior."""
        clock = {"now": 1000.0}
        monkeypatch.setattr(
            "pysymex.analysis.runtime.cache.invalidation.time.time", lambda: clock["now"]
        )
        cache = TieredCache(db_path=tmp_path / "cache.db")
        invalidator = SmartInvalidator(cache)
    ...
    def test_zero_age_time_rule_is_stale_immediately(
        self,
        tmp_path: Path,
        ...
        cache = TieredCache(db_path=tmp_path / "cache.db")
        invalidator = SmartInvalidator(cache)
        rule = InvalidationRule("MODULE:*", InvalidationStrategy.TIME_BASED, max_age_seconds=0)
        invalidator.add_rule(rule)
        key = CacheKey(CacheKeyType.MODULE, "foo")
    ...
    def test_dependency_invalidation_removes_changed_key_and_dependents(
        self,
        tmp_path: Path,
    ) -> None:
        """Dependency invalidation removes the changed key and its transitive dependents."""
        cache = TieredCache(db_path=tmp_path / "cache.db")
        invalidator = SmartInvalidator(cache)
        parent = CacheKey(CacheKeyType.MODULE, "parent")
        child = CacheKey(CacheKeyType.FUNCTION, "child")
        invalidator.add_rule(InvalidationRule("MODULE:*", InvalidationStrategy.DEPENDENCY))
        cache.put(parent, "old-parent")
        cache.put(child, "old-child", dependencies=[parent])

        invalidated = invalidator.on_change(parent)

        assert parent.to_string() in invalidated
        assert child.to_string() in invalidated
...
class TestFileCache:
    """Test suite for pysymex.analysis.runtime.cache.invalidation.FileCache."""
    ...
    def test_get_or_analyze_caches_none_result(self, tmp_path: Path) -> None:
        cache = TieredCache(db_path=tmp_path / "cache.db")
        fc = FileCache(cache)
        file_path = tmp_path / "test.py"
        file_path.write_bytes(b"content")
    ...
    def test_invalidate_missing_untracked_file_is_noop(self, tmp_path: Path) -> None:
        cache = TieredCache(db_path=tmp_path / "cache.db")
        fc = FileCache(cache)

        fc.invalidate(tmp_path / "deleted.py")
