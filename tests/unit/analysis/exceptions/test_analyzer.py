...
from pysymex.analysis.exceptions.analyzer import (
    ExceptionASTAnalyzer, ExceptionBytecodeAnalyzer, UncaughtExceptionAnalyzer,
    ExceptionChainAnalyzer, ExceptionAnalyzer
)
...
class TestExceptionBytecodeAnalyzer:
    ...
    def test_analyze(self) -> None:
        ...
        # Currently the bytecode analyzer doesn't yield warnings on its own
        assert len(warnings) == 0
...
class TestUncaughtExceptionAnalyzer:
    ...
    def test_analyze(self) -> None:
        ...
        code = f.__code__ # type: ignore[arg-type]
        potential = analyzer.analyze(code)
        # getattr is in the code and uncaught
...
class TestExceptionChainAnalyzer:
    ...
    def test_analyze_source(self) -> None:
        ...
        assert isinstance(warnings, list)
        assert len(warnings) == 0 # not implemented yet
