...
from pysymex.analysis.dead_code.core import (
    UnreachableCodeDetector, UnusedVariableDetector, DeadStoreDetector,
    UnusedFunctionDetector, UnusedParameterDetector, UnusedImportDetector,
...
class MockInstr:
    def __init__(self, opname: str, offset: int, argval: object = None, is_jump_target: bool = False, starts_line: int | None = None) -> None:
        self.opname = opname
        self.offset = offset
        self.argval = argval
...
class TestUnreachableCodeDetector:
    ...
    def test_detect(self, mock_get_instr) -> None:
        ...
        mock_get_instr.return_value = [
            ...
            MockInstr("LOAD_GLOBAL", 4, "print", starts_line=11), # Unreachable
...
class TestUnusedVariableDetector:
    ...
    def test_detect(self) -> None:
        ...
        code = make_unused_var_code() # type: ignore[arg-type]
...
class TestDeadStoreDetector:
    ...
    def test_detect(self) -> None:
        ...
        code = make_dead_store_code() # type: ignore[arg-type]
        issues = detector.detect(code)
        assert len(issues) == 1
...
class TestUnusedParameterDetector:
    ...
    def test_detect(self) -> None:
        ...
        code = make_unused_param_code() # type: ignore[arg-type]
        issues = detector.detect(code)
        assert len(issues) == 1
