...
from pysymex.analysis.detectors.filter import (
    IssueLike, Confidence, AssertionContext, FilterResult,
    is_typing_false_positive, is_type_checking_block_issue,
    detect_assertion_context, calculate_confidence, filter_issue,
    filter_issues, deduplicate_issues
)
from pysymex.analysis.detectors.base import IssueKind

class MockIssue:
    def __init__(self, kind: IssueKind, message: str, function_name: str | None = None,
                 model: object | None = None, line_number: int | None = None, pc: int = 0) -> None:
        self._kind = kind
    ...
    @property
    def kind(self) -> IssueKind: return self._kind
    @property
    def message(self) -> str: return self._message
    @property
    def function_name(self) -> str | None: return self._function_name
    @property
    def model(self) -> object | None: return self._model
    @property
    def line_number(self) -> int | None: return self._line_number
    @property
    def pc(self) -> int: return self._pc
...
class TestIssueLike:
    ...
    def test_kind(self) -> None:
        ...
        assert issue.kind == IssueKind.UNKNOWN
...
def test_is_typing_false_positive() -> None:
    ...
    assert is_typing_false_positive(MockIssue(IssueKind.UNKNOWN, "Attempting to subscript Callable")) is True
...
def test_is_type_checking_block_issue() -> None:
    ...
    assert is_type_checking_block_issue(MockIssue(IssueKind.UNKNOWN, "in TYPE_CHECKING block")) is True
