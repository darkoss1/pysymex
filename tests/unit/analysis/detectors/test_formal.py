...
from pysymex.analysis.detectors.formal import (
    DetectorFormalSpec, ProofObligationResult, StatisticalResult,
    MutationResult, OracleResult, specs, prove_smt_obligations,
    run_property_validation, run_mutation_analysis,
    run_oracle_differential_validation, build_machine_checkable_report
)
...
@patch("pysymex.analysis.detectors.formal._PURE_CHECK_DIVISION_BY_ZERO", return_value=None)
@patch("pysymex.analysis.detectors.formal._PURE_CHECK_INDEX_BOUNDS", return_value=None)
@patch("pysymex.analysis.detectors.formal._PURE_CHECK_NONE_DEREF", return_value=None)
...
def test_run_property_validation(m1, m2, m3, m4) -> None:
    ...
    # We mock SymbolicValue to avoid deep calls
    with patch("pysymex.analysis.detectors.formal.SymbolicValue.symbolic") as m_sym, \
...
@patch("pysymex.analysis.detectors.formal.run_mutation_analysis", return_value=[MutationResult("foo", 1, 1, 1.0)])
