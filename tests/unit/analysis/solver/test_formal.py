...
from pysymex.analysis.solver.formal import (
    ...
    function_checklist, run_opcode_differential_validation, run_mutation_robustness,
    build_done_gate_report
)

class TestFunctionChecklistItem:
    """Test suite for pysymex.analysis.solver.formal.FunctionChecklistItem."""
...
class TestDifferentialResult:
    """Test suite for pysymex.analysis.solver.formal.DifferentialResult."""
    def test_initialization(self) -> None:
        """Test basic initialization."""
        res = DifferentialResult("name", 10, 0, 0.0, 0.05)
        assert res.samples == 10
        assert res.mismatches == 0

class TestMutationResult:
    """Test suite for pysymex.analysis.solver.formal.MutationResult."""
    def test_initialization(self) -> None:
        """Test basic initialization."""
...
def test_run_opcode_differential_validation() -> None:
    """Test run_opcode_differential_validation behavior."""
    res = run_opcode_differential_validation(samples=2, seed=42)
    assert len(res) == 2
...
def test_build_done_gate_report() -> None:
    ...
    report = build_done_gate_report(samples=2, seed=42)
