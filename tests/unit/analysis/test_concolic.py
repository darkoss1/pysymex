...
from pysymex.analysis.concolic import (
    ConcreteInput, BranchRecord, ExecutionTrace, ConcolicExecutor,
    ConcolicResult, GenerationalSearch
)
...
class TestExecutionTrace:
    ...
    def test_path_hash(self) -> None:
        ...
        inp = ConcreteInput(values={})
        trace = ExecutionTrace(input=inp, branches=[BranchRecord(pc=0, condition=z3.BoolVal(True), taken=True)])
...
class TestConcolicExecutor:
    ...
    def test_execute(self) -> None:
        ...
        # We mock _execute_concrete and _expand_worklist avoiding actual AST execution requirements in unit tests
        with patch.object(exec, "_execute_concrete") as mock_exec, \
