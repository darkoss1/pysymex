...
class TestLoopSummarizer:
    ...
    def test_summarize(self) -> None:
        ...
        # Create a proper mock state with required attributes
        mock_state = Mock()
        mock_state.locals = {}
        mock_state.stack = []
        mock_state.memory = {}
        mock_state.path_constraints = []
        mock_state.current_instructions = []  # Required by _detect_accumulator_effects

        assert ls.summarize(loop, mock_state) is None
        ...
        summary = ls.summarize(loop, mock_state)
...
class TestLoopInvariantGenerator:
    ...
    def test_verify_invariant(self, mock_is_sat) -> None:
        ...
        loop = LoopInfo(header_pc=10, back_edge_pc=20, exit_pcs={30}, body_pcs={10, 20})
        loop.induction_vars = {}
        assert lig.verify_invariant(inv, loop, state) is True
