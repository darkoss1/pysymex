...
class TestResourceTracker:
    ...
    def test_init_with_defaults(self) -> None:
        """ResourceTracker defaults to automatic host exploration."""
        assert ResourceTracker().limits.max_paths is None
    ...
    def test_start_samples_memory_once(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """start() should not query memory twice for baseline plus first sample."""
        samples = 0

        def fake_memory_usage(logger: object) -> float:
            _ = logger
            nonlocal samples
            samples += 1
            return 100.0
        ...
        monkeypatch.setattr(
            "pysymex._internal.limits.tracker.current_memory_usage_mb",
            fake_memory_usage,
        )
    ...
    def test_record_path_does_not_emit_stats_event(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "pysymex._internal.limits.tracker.emit",
            record_emit,
            raising=False,
        )
    ...
    def test_automatic_limits_do_not_raise(self) -> None:
        """Absent elective limits never stop host exploration."""
        t = ResourceTracker()
        t.start()
        for _ in range(1001):
            t.record_path()
