...
from pysymex.execution.opcodes.py311 import collections
...
def test_handle_list_to_tuple() -> None:
    """Test handle_list_to_tuple behavior."""
    state = VMState(stack=[SymbolicValue.from_const([1, 2, 3])], pc=0)
    collections.handle_list_to_tuple(_instr("LIST_TO_TUPLE"), state, OpcodeDispatcher())
    assert state.pc == 1
...
def test_handle_collection_update() -> None:
    ...
    result = collections.handle_collection_update(
        _instr("DICT_UPDATE", 1), state, OpcodeDispatcher()
    )
...
def test_handle_unpack_sequence() -> None:
    ...
    result = collections.handle_unpack_sequence(
        _instr("UNPACK_SEQUENCE", 2), state, OpcodeDispatcher()
    )
