...
def test_handle_prep_reraise_star() -> None:
    """Test handle_prep_reraise_star behavior."""
    state = VMState(
        stack=[
    ...
    exceptions.handle_prep_reraise_star(_instr("PREP_RERAISE_STAR"), state, OpcodeDispatcher())
    assert state.pc == 1
    top = state.peek()
