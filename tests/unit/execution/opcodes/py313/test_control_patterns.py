...
def test_handle_match_mapping() -> None:
    ...
    control.handle_match_mapping(instr("MATCH_MAPPING"), state, OpcodeDispatcher())
...
def test_handle_match_sequence() -> None:
    ...
    control.handle_match_sequence(instr("MATCH_SEQUENCE"), state, OpcodeDispatcher())
    assert isinstance(state.peek(), SymbolicValue)
...
def test_handle_match_keys() -> None:
    ...
    res = control.handle_match_keys(instr("MATCH_KEYS"), state, OpcodeDispatcher())
...
def test_handle_match_keys_rejects_missing_key_on_heap_backed_symbolic_dict() -> None:
    ...
    control.handle_match_keys(instr("MATCH_KEYS"), state, OpcodeDispatcher())
    result = state.peek()
...
def test_handle_match_class() -> None:
    ...
    res = control.handle_match_class(instr("MATCH_CLASS", 0), state, OpcodeDispatcher())
    assert res is not None
