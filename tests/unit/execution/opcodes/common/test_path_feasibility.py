...
def test_path_is_sat_short_circuits_literal_false_without_solver() -> None:
    ...
    with patch(
        ...
        assert PathSatisfiability.is_sat([x > 0, z3.BoolVal(False)]) is False
...
def test_path_is_sat_strips_literal_true_before_solver() -> None:
    ...
    with patch(
        ...
        assert PathSatisfiability.is_sat([z3.BoolVal(True), x > 0]) is True
...
def test_path_is_sat_short_circuits_negated_true_without_solver() -> None:
    ...
    with patch(
        ...
        assert PathSatisfiability.is_sat([x > 0, z3.Not(z3.BoolVal(True))]) is False
...
def test_path_is_sat_strips_negated_false_before_solver() -> None:
    ...
    with patch(
        ...
        assert PathSatisfiability.is_sat([z3.Not(z3.BoolVal(False)), x > 0]) is True
...
def test_path_is_sat_short_circuits_constant_false_comparison_without_solver() -> None:
    ...
    with patch(
        ...
        assert PathSatisfiability.is_sat([x > 0, z3.IntVal(3) < z3.IntVal(3)]) is False
