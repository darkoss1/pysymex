...
from pysymex.execution.opcodes.common import compare as common_compare
...
def test_handle_common_compare_op_skips_feasibility_for_literal_non_type_error(
    ...
    monkeypatch.setattr(common_compare, "path_is_sat", fail_path_is_sat)
...
def test_handle_common_contains_op_keeps_bool_and_int_projection_consistent() -> None:
    ...
    assert z3.is_true(z3.simplify(true_membership.z3_bool))
    assert z3.is_true(z3.simplify(true_membership.z3_int == 1))
    assert z3.is_false(z3.simplify(false_membership.z3_bool))
    assert z3.is_true(z3.simplify(false_membership.z3_int == 0))
