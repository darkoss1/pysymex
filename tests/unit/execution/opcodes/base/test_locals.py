...
from pysymex.execution.opcodes.base import locals
...
def test_handle_load_fast_load_fast() -> None:
    """Test handle_load_fast_load_fast behavior."""
    state = VMState(local_vars={"a": 1, "b": 2}, pc=0)
    locals.handle_load_fast_load_fast(
        _instr("LOAD_FAST_LOAD_FAST", ("a", "b")), state, OpcodeDispatcher()
    )
...
def test_handle_store_fast_store_fast() -> None:
    """Test handle_store_fast_store_fast behavior."""
    state = VMState(stack=[1, 2], pc=0)
    locals.handle_store_fast_store_fast(
        _instr("STORE_FAST_STORE_FAST", ("a", "b")), state, OpcodeDispatcher()
    )
    assert state.local_vars["a"] == 2
    assert state.local_vars["b"] == 1
...
def test_handle_load_fast_and_clear() -> None:
    ...
    locals.handle_load_fast_and_clear(
        _instr("LOAD_FAST_AND_CLEAR", "x"), state, OpcodeDispatcher()
    )
    assert state.peek() == 15
    assert state.local_vars["x"] is UNBOUND


def test_handle_store_fast_load_fast() -> None:
    """Test handle_store_fast_load_fast behavior."""
    state = VMState(stack=[91], local_vars={"b": 2}, pc=0)
    locals.handle_store_fast_load_fast(
        _instr("STORE_FAST_LOAD_FAST", ("a", "b")), state, OpcodeDispatcher()
    )
    assert state.local_vars["a"] == 91
    assert state.peek() == 2


def test_handle_store_fast_maybe_null() -> None:
    """Test handle_store_fast_maybe_null behavior."""
    state = VMState(stack=[7], pc=0)
    locals.handle_store_fast_maybe_null(
        _instr("STORE_FAST_MAYBE_NULL", "x"), state, OpcodeDispatcher()
    )
    assert state.local_vars["x"] == 7
