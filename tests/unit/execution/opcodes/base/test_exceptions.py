...
from pysymex.execution.opcodes.base import exceptions
...
def test_handle_get_yield_from_iter() -> None:
    ...
    state = VMState(stack=[1], pc=0)
    with pytest.raises(NameError):
        exceptions.handle_get_yield_from_iter(
            _instr("GET_YIELD_FROM_ITER"), state, OpcodeDispatcher()
        )
...
def test_handle_exit_init_check() -> None:
    ...
    state = VMState(stack=[1], pc=0)
    exceptions.handle_exit_init_check(_instr("EXIT_INIT_CHECK"), state, OpcodeDispatcher())
    assert state.stack == []
...
def test_handle_interpreter_exit() -> None:
    ...
    result = exceptions.handle_interpreter_exit(_instr("INTERPRETER_EXIT"), VMState(), OpcodeDispatcher())
    assert result.terminal is True
...
def test_handle_raise_varargs() -> None:
    ...
    result = exceptions.handle_raise_varargs(_instr("RAISE_VARARGS", 1, offset=0), state, OpcodeDispatcher())
    assert result.terminal is True
