...
from pysymex.execution.opcodes.base import control
...
def test_handle_pop_jump_if_false() -> None:
    """Test handle_pop_jump_if_false behavior."""
    dispatcher = OpcodeDispatcher()
    dispatcher.set_instructions([_instr("NOP", offset=0), _instr("NOP", offset=10)])
    state = VMState(stack=[False], pc=0)
    result = control.handle_pop_jump_if_false(
        _instr("POP_JUMP_IF_FALSE", 10, offset=0), state, dispatcher
    )
    assert len(result.new_states) >= 1


def test_handle_pop_jump_if_true() -> None:
    """Test handle_pop_jump_if_true behavior."""
    dispatcher = OpcodeDispatcher()
    dispatcher.set_instructions([_instr("NOP", offset=0), _instr("NOP", offset=10)])
    state = VMState(stack=[True], pc=0)
    result = control.handle_pop_jump_if_true(
        _instr("POP_JUMP_IF_TRUE", 10, offset=0), state, dispatcher
    )
    assert len(result.new_states) >= 1
...
def test_handle_jump_or_pop() -> None:
    ...
    dispatcher.set_instructions([_instr("NOP", offset=0), _instr("NOP", offset=8)])
    state = VMState(stack=[True], pc=0)
    result = control.handle_jump_or_pop(_instr("JUMP_IF_TRUE_OR_POP", 8, offset=0), state, dispatcher)
    assert len(result.new_states) >= 1
...
def test_handle_match_class() -> None:
    ...
    with pytest.raises(NameError):
        control.handle_match_class(_instr("MATCH_CLASS", 0), state, OpcodeDispatcher())
