...
from pysymex.plugins.base import (
    PluginType, PluginPriority, PluginMetadata, Plugin,
    DetectorPlugin, HandlerPlugin, HookPoint, HookPlugin,
    PluginRegistry, PluginLoader, PluginConfig, PluginManagerConfig,
    PluginManager
)
...
class DummyDetectorPlugin(DetectorPlugin):
    ...
    def get_detector(self) -> object:  # type: ignore[override]
...
class DummyHandlerPlugin(HandlerPlugin):
    ...
    def get_handlers(self) -> dict[str, object]: # type: ignore[override]
...
class DummyHookPlugin(HookPlugin):
    ...
    def get_hooks(self) -> dict[str, object]: # type: ignore[override]
        """Get the dummy hooks."""
...
class TestPluginRegistry:
    ...
    def test_activate_all(self) -> None:
        ...
        registry = PluginRegistry()
        plugin = DummyPlugin()
        plugin.activate = MagicMock()  # type: ignore[method-assign] # Mocking for test
        registry.register(plugin)
        engine = cast("SymbolicExecutor", MagicMock())
    ...
    def test_deactivate_all(self) -> None:
        ...
        registry = PluginRegistry()
        plugin = DummyPlugin()
        plugin.deactivate = MagicMock()  # type: ignore[method-assign] # Mocking for test
        registry.register(plugin)
        engine = cast("SymbolicExecutor", MagicMock())
    ...
    def test_register_hook(self) -> None:
        ...
        assert "custom_hook" in registry._hooks  # type: ignore[private-usage]
        assert registry._hooks["custom_hook"] == [handler]  # type: ignore[private-usage]
...
class TestPluginLoader:
    ...
    def test_add_search_path(self) -> None:
        ...
        assert p in loader._search_paths  # type: ignore[private-usage]
